...
const App: React.FC = () => {
  ...
  // (Removed duplicate local obsServiceInstance and setObsServiceInstance state, now using Zustand)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setData({
        scenes: scenesData.scenes.map((s: OBSScene) => ({
      ...
      if (currentProgramSceneData.currentProgramSceneName) {
        ...
        setData({
          sources: sourcesData.sceneItems.map((item: OBSSource) => ({
