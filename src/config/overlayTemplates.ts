...
window.EmoteWall = {
  ...
  {
    templateName: 'Generative Follower Alert',
    customizations: {
      fontSize: '18px',
      colors: {
        primary: 'var(--accent)',
    ...
    generatedCode: {
      html: `
<div id="follower-alert" class="alert-hidden">
  <img id="follower-image" src="" alt="AI Generated Image" />
  <div class="alert-content">
    <h3 id="follower-name"></h3>
    <p id="follower-message"></p>
  </div>
</div>
      `,
      css: `
#follower-alert {
  ...
  display: flex;
  align-items: center;
  box-shadow: 0 4px 20px var(--accent-glow, rgba(0, 255, 255, 0.3));
  opacity: 0;
  transition: all 0.5s ease;
}
...
#follower-image {
  ...
  border-radius: 50%;
  margin-right: 20px;
  border: 3px solid var(--accent, #00ffff);
}
...
@keyframes fadeInDown {
  ...
  to { transform: translate(-50%, 0); opacity: 1; }
      ...
      `,
      js: `
function showFollowerAlert(username, message, imageUrl) {
  const alert = document.getElementById('follower-alert');
  document.getElementById('follower-name').textContent = username;
  document.getElementById('follower-message').textContent = message;
  document.getElementById('follower-image').src = imageUrl;
...
window.addEventListener('message', (event) => {
  if (event.data.type === 'newFollower') {
    showFollowerAlert(event.data.username, event.data.message, event.data.imageUrl);
  }
});
      `
    }
