export function highlightJsonSyntax(rawJsonString: string): string {
  ...
  htmlEscapedJsonString = htmlEscapedJsonString
    ...
    .replace(
      /(?<!\w)([-+]?\d*\.?\d+([eE][-+]?\d+)?)(?!\w)/g,
      '<span class="text-orange-800">$1</span>',
    );
...
export function applyInlineMarkdown(text: string): string {
  ...
  html = html.replace(
    /\{\{glow:([^}]+)\}\}/g,
    '<span class="text-primary font-semibold animate-pulse drop-shadow-md" style="text-shadow: 0 0 8px currentColor, 0 0 16px currentColor;">$1</span>',
  );
  html = html.replace(
    /\{\{glow-green:([^}]+)\}\}/g,
    '<span class="text-green-400 font-semibold animate-pulse drop-shadow-md" style="text-shadow: 0 0 8px #4ade80, 0 0 16px #4ade80;">$1</span>',
  );
  html = html.replace(
    /\{\{glow-red:([^}]+)\}\}/g,
    '<span class="text-red-400 font-semibold animate-pulse drop-shadow-md" style="text-shadow: 0 0 8px #f87171, 0 0 16px #f87171;">$1</span>',
  );
  html = html.replace(
    /\{\{glow-blue:([^}]+)\}\}/g,
    '<span class="text-blue-400 font-semibold animate-pulse drop-shadow-md" style="text-shadow: 0 0 8px #60a5fa, 0 0 16px #60a5fa;">$1</span>',
  );
  html = html.replace(
    /\{\{glow-yellow:([^}]+)\}\}/g,
    '<span class="text-yellow-400 font-semibold animate-pulse drop-shadow-md" style="text-shadow: 0 0 8px #facc15, 0 0 16px #facc15;">$1</span>',
  );
  html = html.replace(
    /\{\{glow-purple:([^}]+)\}\}/g,
    '<span class="text-purple-400 font-semibold animate-pulse drop-shadow-md" style="text-shadow: 0 0 8px #c084fc, 0 0 16px #c084fc;">$1</span>',
  );

  // Color effects (contextual styling)
  html = html.replace(
    /\{\{success:([^}]+)\}\}/g,
    '<span class="text-green-600 font-medium bg-green-50 px-2 py-0.5 rounded-md border border-green-200">✅ $1</span>',
  );
  html = html.replace(
    /\{\{error:([^}]+)\}\}/g,
  ...
  html = html.replace(
    /\{\{warning:([^}]+)\}\}/g,
    '<span class="text-yellow-600 font-medium bg-yellow-50 px-2 py-0.5 rounded-md border border-yellow-200">⚠️ $1</span>',
  );
  html = html.replace(
    /\{\{info:([^}]+)\}\}/g,
    '<span class="text-blue-600 font-medium bg-blue-50 px-2 py-0.5 rounded-md border border-blue-200">ℹ️ $1</span>',
  );
  html = html.replace(
    /\{\{tip:([^}]+)\}\}/g,
    '<span class="text-purple-600 font-medium bg-purple-50 px-2 py-0.5 rounded-md border border-purple-200">💡 $1</span>',
  );

  // Contextual OBS styling
  html = html.replace(
    /\{\{obs-action:([^}]+)\}\}/g,
    '<span class="text-orange-600 font-medium bg-orange-50 px-2 py-0.5 rounded-md border border-orange-200 font-mono">🎬 $1</span>',
  );
  html = html.replace(
    /\{\{stream-live:([^}]+)\}\}/g,
  ...
  html = html.replace(
    /\{\{stream-offline:([^}]+)\}\}/g,
    '<span class="text-gray-500 font-medium bg-gray-100 px-2 py-0.5 rounded-md border border-gray-300">⚫ $1</span>',
  );

  // Rainbow text effect
  html = html.replace(
    /\{\{rainbow:([^}]+)\}\}/g,
    '<span class="font-bold bg-gradient-to-r from-red-500 via-yellow-500 via-green-500 via-blue-500 to-purple-500 bg-clip-text text-transparent animate-pulse">$1</span>',
  );
  ...
  html = html.replace(
    /\{\{highlight:([^}]+)\}\}/g,
    '<span class="bg-yellow-200 text-yellow-900 px-1 py-0.5 rounded font-medium">$1</span>',
  );
  html = html.replace(
    /\{\{highlight-green:([^}]+)\}\}/g,
    '<span class="bg-green-200 text-green-900 px-1 py-0.5 rounded font-medium">$1</span>',
  );
  html = html.replace(
    /\{\{highlight-blue:([^}]+)\}\}/g,
    '<span class="bg-blue-200 text-blue-900 px-1 py-0.5 rounded font-medium">$1</span>',
  );
