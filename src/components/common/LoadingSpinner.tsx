...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({
  ...
  const renderSpinner = () => {
    ...
    switch (variant) {
      case 'pulse':
        return (
          <div className={cn('animate-pulse', sizeClass, colorClass, className)}>
          ...
          <div className={cn('relative', sizeClass, className)}>
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className={cn(
                  'absolute top-0 left-0 w-full h-full border-2 border-current border-t-transparent rounded-full',
                  getSpeedStyles(),
                  colorClass
                )}
                style={{
      ...
      case 'bounce':
        return (
          <div className={cn('animate-bounce', sizeClass, colorClass, className)}>
            <div className="w-full h-full bg-current rounded-full" />
          </div>
        );
      ...
      default:
        return (
          <svg
            className={cn(getSpeedStyles(), sizeClass, colorClass, className)}
