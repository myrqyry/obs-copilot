...
async function verify() {
    ...
    await testEndpoint('Favicon Proxy (via /api/proxy)', '/api/proxy?api=favicon&domain=streamer.bot&sz=16', 200 // Expect 200 if proxy works, even if Google fetch has issues (proxy handles it)
    );
    // Test Generic Image Proxy (/api/image) - expect 500 if URL is invalid/unfetchable
    await testEndpoint('Generic Image Proxy (Bad URL)', '/api/image?url=http://invalid-url-for-testing.com/image.png', 500, // Expecting failure from fetch
    true // Expect JSON error
    );
    // Test Iconfinder SVG Proxy - expect 400 if no URL
    await testEndpoint('Iconfinder SVG Proxy (No URL)', '/api/iconfinder/svg', 400, true // Expect JSON error
    );
    // Test Pexels API (no key, expect error)
    await testEndpoint('Pexels API (No Key)', '/api/pexels?query=nature', 500, // Pexels proxy in proxy.cjs returns 500 if key is missing
    true // Expect JSON error { error: "Pexels API key not provided..." }
    );
    // Test Gemini API (no key, bad request, expect error)
    await testEndpoint('Gemini API (Bad Request)', '/api/gemini?model=gemini-pro', // Missing API key in actual call, POST body also missing
    ...
    true, // Expect JSON error
    'POST', JSON.stringify({ contents: [{ parts: [{ text: "Hello" }] }] }) // Example body
    );
    // Test Chutes API (no key, bad request, expect error)
    await testEndpoint('Chutes API (Bad Request)', '/api/chutes', // Missing API key in actual call
    500, // proxy.cjs returns 500 if CHUTES_API_TOKEN is not set
    true, // Expect JSON error
    'POST', JSON.stringify({ prompt: "test" }) // Example body
    );
    // Test an unknown API endpoint
    await testEndpoint('Unknown API Endpoint', '/api/nonexistentapi', 400, // Or 404 depending on how proxy.cjs handles truly unknown paths vs. known paths with bad params
