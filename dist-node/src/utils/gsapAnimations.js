...
export const uiAnimations = {
     ...
     */
    tabContentTransition: (element, options) => {
        const { direction = 'up', duration = ANIMATION_CONFIG.FAST, delay = 0 } = options || {};
    ...
    staggerChildren: (parent, options) => {
        const { stagger = 0.1, duration = ANIMATION_CONFIG.FAST, from = 'start' } = options || {};
...
export const textSplitAnimations = {
     ...
     */
    flyToSend: (element, options) => {
        const { duration = 0.8, stagger = 0.03, ease = 'power2.in', direction = 'right', onComplete } = options || {};
        const target = typeof element === 'string' ? document.querySelector(element) : element;
        if (!target)
            return null;
     ...
     */
    flyFromSend: (element, text, options) => {
        const { duration = 0.6, stagger = 0.02, ease = 'back.out(1.7)', onComplete } = options || {};
        const target = typeof element === 'string' ? document.querySelector(element) : element;
        if (!target)
            return null;
     ...
     */
    resetTextSplit: (element, originalText) => {
        const target = typeof element === 'string' ? document.querySelector(element) : element;
        if (!target)
            return;
    ...
    typewriter: (element, text, options) => {
        const { duration = 0.05, ease = 'none', onComplete } = options || {};
        const target = typeof element === 'string' ? document.querySelector(element) : element;
        if (!target)
            return null;
        target.textContent = text;
...
export const triggerTextSplitOnSend = (inputElement, sendButtonElement, onComplete) => {
