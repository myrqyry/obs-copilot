...
export function detectChoiceQuestion(text, obsData) {
...
export function extractChoicesFromText(text) {
    const choices = [];
...
export function detectObsChoiceQuestion(text, obsData) {
    ...
    if ((isAmbiguous && lowercaseText.includes('scene')) ||
        ...
        const scenes = obsData.scenes?.map((scene) => scene.sceneName)
            .filter((name) => name !== obsData.currentProgramScene) || [];
        if (scenes.length > 1) {
    ...
    if ((isAmbiguous && lowercaseText.includes('source')) ||
        ...
        const sources = obsData.sources?.map((source) => source.sourceName) || [];
        if (sources.length > 1) {
    ...
    if ((isAmbiguous && lowercaseText.includes('camera')) ||
        ...
        const cameraSources = obsData.sources?.filter((source) => source.inputKind === 'dshow_input' ||
            source.sourceName.toLowerCase().includes('camera') ||
            source.sourceName.toLowerCase().includes('webcam') ||
            source.sourceName.toLowerCase().includes('cam')).map((source) => source.sourceName) || [];
    ...
    if ((isAmbiguous && lowercaseText.includes('text source')) ||
        ...
        const textSources = obsData.sources?.filter((source) => source.inputKind === 'text_gdiplus_v2' || source.inputKind === 'text_ft2_source_v2' ||
            source.sourceName.toLowerCase().includes('text')).map((source) => source.sourceName) || [];
    ...
    if ((isAmbiguous && lowercaseText.includes('audio')) ||
        ...
        const audioSources = obsData.sources?.filter((source) => source.inputKind?.includes('audio') ||
            source.inputKind?.includes('wasapi') ||
            source.inputKind?.includes('dshow') ||
            source.sourceName.toLowerCase().includes('audio') ||
            source.sourceName.toLowerCase().includes('mic')).map((source) => source.sourceName) || [];
    ...
    if ((isAmbiguous && lowercaseText.includes('screen')) ||
        ...
        const screenSources = obsData.sources?.filter((source) => source.inputKind === 'monitor_capture' ||
            ...
            source.sourceName.toLowerCase().includes('monitor')).map((source) => source.sourceName) || [];
