...
export function buildObsSystemMessage(obsData, hotkeys) {
    ...
    if (hotkeys && hotkeys.length > 0) {
        const hotkeyNames = hotkeys.map((h) => h.hotkeyName || h.name).filter(Boolean).slice(0, 20); // Limit to first 20
...
export function buildStreamerBotSystemMessage() {
...
export function buildMarkdownStylingSystemMessage() {
    return `
