...
export const tabAnimations = {
     ...
     */
    animateTabEnter: (element, direction = 'left') => {
     ...
     */
    animateTabExit: (element, direction = 'right') => {
        const timeline = gsap.timeline();
        timeline.to(element, {
            opacity: 0,
            x: direction === 'left' ? -30 : 30,
     ...
     */
    animateTabButton: (element, isActive) => {
        const timeline = gsap.timeline();
        if (isActive) {
            timeline
                .to(element, {
     ...
     */
    animateTabChildren: (container, delay = 0.1) => {
     ...
     */
    animateConnectionStatus: (element, isConnected) => {
        const timeline = gsap.timeline();
        if (isConnected) {
            timeline
                .to(element, {
     ...
     */
    animateMessage: (element, fromUser = false) => {
        const timeline = gsap.timeline();
        gsap.set(element, {
            opacity: 0,
            x: fromUser ? 30 : -30,
     ...
     */
    animateStatusPulse: (element, color = 'currentColor') => {
...
export class TabTransitionManager {
    currentElement = null;
    isAnimating = false;
    async switchTab(newElement, direction = 'left') {
        if (this.isAnimating)
            return;
