...
import { AppTab, // Add ChatMessage here
catppuccinAccentColorsHexMap, catppuccinSecondaryAccentColorsHexMap, catppuccinChatBubbleColorsHexMap, } from './types';
import { OBSWebSocketService } from './services/obsService';
import { StreamerBotService } from './services/streamerBotService';
import { useAppStore } from './store/appStore'; // Import AppState
...
const App = () => {
    ...
    const [streamerBotAddress, setStreamerBotAddress] = useState(persistedConnectionSettings.streamerBotAddress || 'localhost');
    const [streamerBotPort, setStreamerBotPort] = useState(persistedConnectionSettings.streamerBotPort || '8080');
    ...
    const tabOrder = [AppTab.GEMINI, AppTab.OBS_STUDIO, AppTab.STREAMING_ASSETS, AppTab.CREATE, AppTab.SETTINGS, AppTab.CONNECTIONS, AppTab.ADVANCED];
    ...
    useEffect(() => {
        ...
        const adjustForExtraDarkMode = (rgb) => {
            if (!extraDarkMode)
                return rgb;
    ...
    const { extraDarkMode: extraDarkModeFromStore } = useAppStore((state) => state.userSettings);
    useEffect(() => {
        if (headerRef.current) {
            setHeaderHeight(headerRef.current.offsetHeight);
        }
    ...
    useEffect(() => {
        ...
        const hasInitialMessage = geminiMessagesRef.current.some((m) => m.role === 'system' && (m.text.includes("Gemini Assistant initialized") ||
    ...
    const fetchData = useCallback(async (obsService) => {
        const serviceToUse = obsService || obsServiceInstance;
        if (!serviceToUse)
            return;
        try {
            const [scenesData, currentProgramSceneData, streamStatusData, recordStatusData, videoSettingsData, streamerUsernameData] = await Promise.all([
            ...
            if (currentProgramSceneData.currentProgramSceneName) {
                ...
                sources = sourcesData.sceneItems.map((item) => ({
    ...
    const handleConnect = useCallback(async (address, password) => {
        actions.setConnecting();
        // Fix: Support both ESM and CJS builds of obs-websocket-js
        let NewOBSWebSocket = OBSWebSocket;
        ...
        if (OBSWebSocket && typeof OBSWebSocket === 'object' && 'default' in OBSWebSocket) {
            NewOBSWebSocket = OBSWebSocket.default;
        }
    ...
    const handleStreamerBotAddressChange = useCallback((address) => {
        setStreamerBotAddress(address);
        if (isStorageAvailable()) {
            saveConnectionSettings({ streamerBotAddress: address });
        }
    }, []);
    const handleStreamerBotPortChange = useCallback((port) => {
    ...
    useEffect(() => {
        const attemptAutoConnect = async () => {
            if (isConnected || isConnecting || obs)
                return; // Don't auto-connect if already connected/connecting
            if (isStorageAvailable()) {
    ...
    useEffect(() => {
        if (tabContentRef.current) {
            ...
            gsap.fromTo(tabContentRef.current, { opacity: 0 }, {
                opacity: 1,
                duration: 0.2,
                ease: 'power2.out',
                onComplete: () => {
    ...
    const renderTabContent = () => {
        const envApiKey = process.env.VITE_GEMINI_API_KEY || process.env.API_KEY;
        switch (activeTab) {
            case AppTab.CONNECTIONS:
                return (<ConnectionPanel onConnect={handleConnect} onDisconnect={handleDisconnect} isConnected={isConnected} isConnecting={isConnecting} defaultUrl={DEFAULT_OBS_WEBSOCKET_URL} error={connectError} geminiApiKey={geminiApiKey} envGeminiApiKey={envApiKey} onGeminiApiKeyChange={actions.setGeminiApiKey} isGeminiClientInitialized={isGeminiClientInitialized} geminiInitializationError={geminiInitializationError} accentColorName={theme.accent} streamerBotAddress={streamerBotAddress} setStreamerBotAddress={handleStreamerBotAddressChange} streamerBotPort={streamerBotPort} setStreamerBotPort={handleStreamerBotPortChange} onStreamerBotConnect={handleStreamerBotConnect} onStreamerBotDisconnect={handleStreamerBotDisconnect} isStreamerBotConnected={isStreamerBotConnected} isStreamerBotConnecting={isStreamerBotConnecting}/>);
            case AppTab.OBS_STUDIO:
                return (<>
                        {!isConnected || !obsServiceInstance ? (<p className="text-center text-muted-foreground mt-6">
                                <span className="emoji">🔗</span> Please connect to OBS WebSocket in the Connections tab to begin.
                            </p>) : (<ObsMainControls obsService={obsServiceInstance} onRefreshData={fetchData} setErrorMessage={setErrorMessage} accentColorName={theme.accent}/>)}
                    </>);
            ...
            case AppTab.SETTINGS:
                return (<ObsSettingsPanel selectedAccentColorName={theme.accent} selectedSecondaryAccentColorName={theme.secondaryAccent} selectedUserChatBubbleColorName={theme.userChatBubble} selectedModelChatBubbleColorName={theme.modelChatBubble} flipSides={flipSides} actions={actions}/>);
            case AppTab.ADVANCED:
                return <AdvancedPanel />;
            case AppTab.GEMINI:
                return (<GeminiChat geminiApiKeyFromInput={envApiKey || geminiApiKey} streamerBotService={streamerBotService} onRefreshData={fetchData} setErrorMessage={setErrorMessage} chatInputValue={geminiChatInput} onChatInputChange={setGeminiChatInput} accentColorName={theme.accent} messages={geminiMessages} onAddMessage={actions.addMessage} isGeminiClientInitialized={isGeminiClientInitialized} geminiInitializationError={geminiInitializationError} onSetIsGeminiClientInitialized={actions.setGeminiClientInitialized} onSetGeminiInitializationError={actions.setGeminiInitializationError} activeTab={activeTab} onStreamerBotAction={handleStreamerBotAction}/>);
            default:
                return null;
        }
    };
    return (<div className="h-screen max-h-screen bg-gradient-to-br from-background to-card text-foreground flex flex-col overflow-hidden">
            ...
            const getMobileTabTitle = (tabName) => {
                switch (tabName) {
                    case AppTab.GEMINI: return 'Assistant';
            ...
            <main className="flex-grow overflow-y-auto px-1 pb-1">
                {isConnecting && !isConnected && (<div className="flex justify-center items-center mt-1 text-orange-500">
                        <LoadingSpinner size={4}/>
                        <span className="ml-2 text-sm">Connecting to OBS...</span>
                    </div>)}

                {errorMessage && (<Modal title="❗ Application Error" onClose={() => setErrorMessage(null)} accentColorName={theme.accent}>
                        <p>{errorMessage}</p>
                    </Modal>)}
