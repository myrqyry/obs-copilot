...
export const useAppStore = create((set, get) => {
    // Set the selected audio output device by id
    function setAudioOutputDevice(deviceId) {
        set({ selectedAudioOutputId: deviceId });
    }
    // Prefer env for Gemini API key if not set in persistence
    ...
    const initialState = {
        ...
        geminiMessages: [],
        geminiApiKey: persistedSettings.geminiApiKey || envGeminiApiKey,
        ...
        userSettings: {
            ...
            theme: {
                accent: persistedSettings.theme?.accent || 'mauve',
                secondaryAccent: persistedSettings.theme?.secondaryAccent || 'flamingo',
                userChatBubble: persistedSettings.theme?.userChatBubble || 'blue',
                modelChatBubble: persistedSettings.theme?.modelChatBubble || 'lavender',
            },
        },
    ...
    return {
        ...
        actions: {
            ...
            replaceMessage: (messageId, newMessage) => set((state) => ({
                geminiMessages: state.geminiMessages.map(msg => msg.id === messageId
                    ? { ...newMessage, id: messageId, timestamp: new Date() }
                    : msg)
            })),
            setGeminiApiKey: (key) => {
                // If key is falsy, fallback to env
                const newKey = key || import.meta.env.VITE_GEMINI_API_KEY || '';
                set({ geminiApiKey: newKey });
                // Save only if user provided a key (not when falling back to env)
                ...
                try {
                    switch (action.type) {
                        case 'createInput':
                            ...
                            await obsServiceInstance.createInput(createAction.inputName, createAction.inputKind, createAction.inputSettings, sceneToAddTo, createAction.sceneItemEnabled);
                            actionFeedback += `\n✅ Successfully created input "${createAction.inputName}" of kind "${createAction.inputKind}".`;
                            break;
                        case 'setInputSettings':
                            const setSettingsAction = action;
                            await obsServiceInstance.setInputSettings(setSettingsAction.inputName, setSettingsAction.inputSettings, setSettingsAction.overlay);
                        ...
                        case 'getSourceScreenshot':
                            ...
                            const screenshotData = await obsServiceInstance.getSourceScreenshot(screenshotAction.sourceName, screenshotAction.imageFormat, screenshotAction.imageWidth, screenshotAction.imageHeight, screenshotAction.imageCompressionQuality);
            ...
            updateAutomationRule: (id, updates) => {
                const updatedRules = get().automationRules.map(rule => rule.id === id ? { ...rule, ...updates } : rule);
                set({ automationRules: updatedRules });
                if (isStorageAvailable()) {
                    saveUserSettings({ automationRules: updatedRules });
                }
            ...
            toggleAutomationRule: (id) => {
                const updatedRules = get().automationRules.map(rule => rule.id === id ? { ...rule, enabled: !rule.enabled } : rule);
                set({ automationRules: updatedRules });
                if (isStorageAvailable()) {
                    saveUserSettings({ automationRules: updatedRules });
                }
            ...
            setStreamerBotServiceInstance: (instance) => {
                ...
                const state = get();
                automationService.initialize(state.automationRules, instance, state.actions.handleObsAction, state.actions.addMessage);
            },
            ...
            startMusicGeneration: async (prompt, config) => {
                ...
                // Always fallback to env variable if store value is empty
                const apiKey = get().geminiApiKey || import.meta.env.VITE_GEMINI_API_KEY || '';
                if (!apiKey) {
                    set({ isMusicPlaying: false });
            ...
            playFromQueue: () => {
                ...
                // Set isQueuePlaying to true only when starting the playback chain
                // This prevents multiple calls to playFromQueue from starting new chains
                if (get().isQueuePlaying) {
                    return;
                }
                set({ isQueuePlaying: true });
                const playNextChunk = () => {
                    const currentQueue = get().audioQueue; // Get the latest queue state
                    if (currentQueue.length === 0) {
                        set({ isQueuePlaying: false });
                        return;
                    }
                    const source = audioContext.createBufferSource();
                    source.buffer = currentQueue[0];
                    source.connect(audioContext.destination);
                    source.start();
                    source.onended = () => {
                        set((state) => ({ audioQueue: state.audioQueue.slice(1) }));
                        // Schedule the next chunk playback
                        // Use a microtask to allow React to process state updates before scheduling next playback
                        Promise.resolve().then(() => playNextChunk());
                    };
                };
                playNextChunk(); // Start the playback chain
            },
            pauseMusic: () => {
