 ...
 */
export function safeGsapTo(target, vars) {
    if (prefersReducedMotion()) {
        // Apply final state immediately without animation
        if (vars.scale !== undefined)
            target.style.transform = `scale(${vars.scale})`;
        if (vars.y !== undefined)
            target.style.transform += ` translateY(${vars.y}px)`;
 ...
 */
export function safeGsapSet(target, vars) {
