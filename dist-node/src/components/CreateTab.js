...
const CreateTab = () => {
    ...
    useEffect(() => {
        if (generatedImage && generatedImageRef.current) {
            gsap.fromTo(generatedImageRef.current, { opacity: 0, scale: 0.8, y: 20 }, {
                duration: 0.5,
                opacity: 1,
                scale: 1,
                y: 0,
    ...
    const [miniPlayerTTSUrl, setMiniPlayerTTSUrl] = useState(null);
    const [audioLoading, setAudioLoading] = useState(false);
    const [audioError, setAudioError] = useState(null);
    const [audioProvider, setAudioProvider] = useState('gemini');
    ...
    const geminiApiKey = (userGeminiApiKey && userGeminiApiKey.trim().length > 0)
    ...
    const handleGenerateImage = async () => {
        ...
        try {
            if (imageProvider === 'gemini') {
                ...
                let userChutesApiKey = undefined;
                try {
    ...
    const handleGenerateAudio = async () => {
        ...
        try {
            ...
            for (const line of lines) {
                ...
                if (match)
                    parts.push({ speaker: match[1], text: match[2] });
            }
            if (!parts.length)
                throw new Error('No valid speaker lines found. Use format: Speaker: line');
            ...
            for (const part of parts) {
                ...
                if (!data)
                    throw new Error(`No audio data for ${part.speaker}`);
    ...
    return (<>
            ...
            <div className="flex flex-col gap-4 p-2 md:p-4 w-full max-w-3xl mx-auto">
                ...
                <CollapsibleCard title="Image Generation & Editing" emoji="🖼️" isOpen={openImage} onToggle={() => setOpenImage(!openImage)} accentColor={accentColor}>
                    <CardContent className="px-3 pb-3 pt-2">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 mb-2">
                            ...
                            <label className="text-xs flex flex-col w-32">
                                ...
                                <select value={imageProvider} onChange={e => setImageProvider(e.target.value)} className="glass-input bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150 rounded px-2 py-1 text-xs" disabled={imageLoading}>
                                    <option value="gemini">Gemini 2.0 Flash Preview</option>
                                    <option value="chutes">Flux.1 Dev via chutes.ai</option>
                                </select>
                            </label>
                            <TextInput value={imagePrompt} onChange={e => setImagePrompt(e.target.value)} placeholder="Describe your image..." className="flex-1 text-sm" disabled={imageLoading} variant="glass" size="md"/>
                            <Button variant="primary" size="sm" isLoading={imageLoading} onClick={handleGenerateImage} disabled={imageLoading || !imagePrompt.trim()}>
                                Generate
                            </Button>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-2 items-center">
                            ...
                            {imageProvider === "gemini" && (<>
                                    <label className="text-xs flex flex-col w-32">
                                        ...
                                        <select value={imageStyle} onChange={e => setImageStyle(e.target.value)} className="glass-input bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150 rounded px-2 py-1 text-xs">
                                    ...
                                    <label className="text-xs flex flex-col w-32">
                                        ...
                                        <select value={aspectRatio} onChange={e => setAspectRatio(e.target.value)} className="glass-input bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150 rounded px-2 py-1 text-xs">
                                    ...
                                    <label className="text-xs flex flex-col w-32">
                                        ...
                                        <select value={quality} onChange={e => setQuality(e.target.value)} className="glass-input bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150 rounded px-2 py-1 text-xs">
                                    ...
                                    <label className="text-xs flex flex-col w-32">
                                        ...
                                        <input type="text" value={textOverlay} onChange={e => setTextOverlay(e.target.value)} className="glass-input bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150 rounded px-2 py-1 text-xs" placeholder="Optional text"/>
                                    </label>
                                    <label className="text-xs flex flex-col w-32">
                                        <span className="mb-1 text-ctp-mauve">Number of Images</span>
                                        <input type="number" min={1} max={4} value={numImages} onChange={e => setNumImages(Number(e.target.value))} className="glass-input bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150 rounded px-2 py-1 text-xs"/>
                                    </label>
                                </>)}
                        ...
                        {generatedImage && (<div className="mt-2 flex flex-col items-center">
                                <img ref={generatedImageRef} src={generatedImage} alt="Generated" className="rounded shadow max-w-full max-h-80 border cursor-pointer" onClick={() => setImageModalOpen(true)} title="Click to preview and add to OBS"/>
                        ...
                        {imageModalOpen && generatedImage && (<Modal isOpen={imageModalOpen} onClose={() => setImageModalOpen(false)} title="Generated Image Preview" actions={[
                {
                ...
                },
                {
                    ...
                    onClick: () => {
                        ...
                        {/* Feedback message */}
                        {feedbackMessage && (<div className="fixed bottom-4 right-4 bg-success text-success-foreground p-3 rounded-lg shadow-lg z-50">
                                {feedbackMessage}
                            </div>)}
                    </CardContent>
                </CollapsibleCard>
                ...
                <CollapsibleCard title="Image Editing (Background Removal)" emoji="🪄" isOpen={openImageEdit} onToggle={() => setOpenImageEdit(!openImageEdit)} accentColor={accentColor}>
                    <CardContent className="px-3 pb-3 pt-2">
                        <ImageEditor />
                    </CardContent>
                </CollapsibleCard>
                ...
                <CollapsibleCard title="Speech Generation" emoji="�️" isOpen={openAudio} onToggle={() => setOpenAudio(!openAudio)} accentColor={accentColor}>
                    <CardContent className="px-3 pb-3 pt-2">
                        <div className="text-xs md:text-sm text-muted-foreground mb-2">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 mb-2">
                            ...
                            <label className="text-xs flex flex-col w-32">
                                <span className="mb-1 text-ctp-mauve">Provider</span>
                                <select value={audioProvider} onChange={e => setAudioProvider(e.target.value)} className="glass-input bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150 rounded px-2 py-1 text-xs" disabled={audioLoading}>
                        ...
                        <div className="flex flex-col gap-2 mb-2">
                            <div className="flex flex-wrap gap-2 items-center mb-2">
                                ...
                                {speakers.map((sp, idx) => (<div key={sp.name + idx} className="flex items-center gap-1 border rounded px-2 py-1" style={{ background: sp.color }}>
                                        <input type="text" value={sp.name} onChange={e => handleSpeakerChange(idx, 'name', e.target.value)} className="w-16 text-xs rounded border px-1 bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                                        <select value={sp.voice} onChange={e => handleSpeakerChange(idx, 'voice', e.target.value)} className="text-xs rounded border px-1 bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150">
                                            {GEMINI_TTS_VOICES.map(v => <option key={v} value={v}>{v}</option>)}
                                        </select>
                                        <select value={sp.style} onChange={e => handleSpeakerChange(idx, 'style', e.target.value)} className="text-xs rounded border px-1 bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150">
                                            {VOICE_STYLES.map(s => <option key={s} value={s}>{s ? s : '(default)'}</option>)}
                                        </select>
                                        <input type="color" value={sp.color} onChange={e => handleSpeakerChange(idx, 'color', e.target.value)} title="Speaker color" pattern="#([0-9a-fA-F]{6})" className="rounded border border-ctp-surface1"/>
                            ...
                            <label className="text-xs flex flex-col">
                                ...
                                <textarea value={script} onChange={e => setScript(e.target.value)} rows={4} className="rounded border px-2 py-1 text-xs font-mono bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150" placeholder="Alice: Hello!\nBob: Hi Alice!" disabled={audioLoading || storyLoading}/>
                            </label>
                        ...
                        <div className="flex flex-wrap gap-2 mb-2 items-center">
                            <label className="text-xs flex flex-col w-32">
                                ...
                                <input type="range" min={0.25} max={4.0} step={0.01} value={speakingRate} onChange={e => setSpeakingRate(Number(e.target.value))} className="accent-ctp-mauve"/>
                            ...
                            <label className="text-xs flex flex-col w-32">
                                ...
                                <input type="range" min={-20} max={20} step={0.1} value={pitch} onChange={e => setPitch(Number(e.target.value))} className="accent-ctp-mauve"/>
                            ...
                            <label className="text-xs flex flex-col w-32">
                                ...
                                <input type="range" min={-96} max={16} step={0.1} value={volumeGainDb} onChange={e => setVolumeGainDb(Number(e.target.value))} className="accent-ctp-mauve"/>
                            ...
                            <label className="text-xs flex flex-col w-40">
                                ...
                                <input type="text" value={effectsProfileId} onChange={e => setEffectsProfileId(e.target.value)} className="rounded border px-2 py-1 text-xs bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150" placeholder="(optional)"/>
                            </label>
                        </div>
                        ...
                        {generatedAudio && (<div className="mt-2 flex flex-col items-center" ref={el => {
                if (el && generatedAudio) {
                    // Animate in with GSAP when generatedAudio appears
                    gsap.fromTo(el, { opacity: 0, scale: 0.8, y: 20 }, {
                        duration: 0.5,
                        opacity: 1,
                        scale: 1,
                        y: 0,
                ...
                <CollapsibleCard title="Music Generation (Lyria RealTime)" emoji="🎶" isOpen={openMusic} onToggle={() => setOpenMusic(!openMusic)} accentColor={accentColor}>
                    <CardContent className="px-3 pb-3 pt-2">
                        <div className="text-xs md:text-sm text-muted-foreground mb-2">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 mb-2">
                            <TextInput value={musicPrompt} onChange={e => setMusicPrompt(e.target.value)} placeholder="Describe your music (e.g. 'minimal techno, 120bpm')" className="flex-1 text-sm bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                            <Tooltip content="Randomize prompt">
                                <Button variant="secondary" size="sm" onClick={handleRandomizeMusicPrompt}>
                                    🎲 Randomize
                                </Button>
                            </Tooltip>
                            <Button variant="primary" size="sm" onClick={handleGenerateMusic} disabled={!musicPrompt.trim()}>
                                Generate
                            </Button>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-2 items-center">
                            <label className="text-xs flex flex-col">
                                ...
                                <input type="number" min={60} max={200} value={bpm} onChange={e => setBpm(Number(e.target.value))} className="rounded border px-2 py-1 w-20 text-xs bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                            </label>
                            <label className="text-xs flex flex-col">
                                ...
                                <input type="number" min={0} max={3} step={0.01} value={temperature} onChange={e => setTemperature(Number(e.target.value))} className="rounded border px-2 py-1 w-20 text-xs bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                            </label>
                            <label className="text-xs flex flex-col">
                                ...
                                <input type="number" min={0} max={1} step={0.01} value={density} onChange={e => setDensity(Number(e.target.value))} className="rounded border px-2 py-1 w-20 text-xs bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                            </label>
                            <label className="text-xs flex flex-col">
                                ...
                                <input type="number" min={0} max={1} step={0.01} value={brightness} onChange={e => setBrightness(Number(e.target.value))} className="rounded border px-2 py-1 w-20 text-xs bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                            </label>
                            <label className="text-xs flex flex-col">
                                ...
                                <input type="number" min={0} max={6} step={0.01} value={guidance} onChange={e => setGuidance(Number(e.target.value))} className="rounded border px-2 py-1 w-20 text-xs bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                            </label>
                            <label className="text-xs flex flex-col">
                                ...
                                <select value={scale} onChange={e => setScale(e.target.value)} className="rounded border px-2 py-1 w-36 text-xs bg-ctp-base border-ctp-surface1 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150">
                                    {LYRIA_SCALES.map(s => (<option key={s.value} value={s.value}>{s.label}</option>))}
                                </select>
                            </label>
