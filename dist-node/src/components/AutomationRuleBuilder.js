...
export const AutomationRuleBuilder = ({ isOpen, onClose, initialEventName, editingRule }) => {
    ...
    const addAction = (type) => {
        const newAction = {
            ...
            type,
            data: type === 'obs'
                ? { type: 'setCurrentProgramScene', sceneName: '' }
                : { actionName: '', args: {} }
        };
    ...
    const renderStepIndicator = () => (<div className="flex items-center space-x-2 mb-6">
            {['trigger', 'conditions', 'actions', 'review'].map((step, index) => (<React.Fragment key={step}>
                    <button onClick={() => setCurrentStep(step)} className={cn("flex items-center justify-center w-8 h-8 rounded-full text-sm font-medium transition-colors", currentStep === step
                ? "bg-accent text-accent-foreground"
                : "bg-muted text-muted-foreground hover:bg-muted/80")}>
                        {index + 1}
                    </button>
                    {index < 3 && (<div className={cn("w-8 h-0.5 transition-colors", ['trigger', 'conditions', 'actions', 'review'].indexOf(currentStep) > index
                    ? "bg-accent"
                    : "bg-muted")}/>)}
                </React.Fragment>))}
        </div>);
    const renderTriggerStep = () => (<div className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-foreground mb-2">
                    Rule Name
                </label>
                <TextInput value={ruleName} onChange={(e) => setRuleName(e.target.value)} placeholder="Enter a descriptive name for this rule" className="w-full"/>
            </div>
            ...
            <div>
                ...
                <select value={trigger.eventName} onChange={(e) => setTrigger({ ...trigger, eventName: e.target.value, eventData: {} })} className="w-full border rounded p-2 bg-background text-foreground">
                    <option value="">Select an event...</option>
                    {OBS_EVENT_LIST.map(event => (<option key={event.name} value={event.name}>
                            {event.name}
                        </option>))}
                </select>
                {trigger.eventName && (<p className="text-sm text-muted-foreground mt-1">
                        {OBS_EVENT_LIST.find(e => e.name === trigger.eventName)?.description}
                    </p>)}
            </div>

            {trigger.eventName && EVENT_DATA_CONFIGS[trigger.eventName] && (<div>
                    <label className="block text-sm font-medium text-foreground mb-2">
                        Event Data Filters (Optional)
                    </label>
                    {EVENT_DATA_CONFIGS[trigger.eventName].map(field => (<div key={field.name} className="mb-3">
                            <label className="block text-xs text-muted-foreground mb-1">
                                {field.description || field.name}
                            </label>
                            {field.type === 'select' ? (<select value={trigger.eventData?.[field.name] || ''} onChange={(e) => setTrigger({
                        ...trigger,
                        eventData: { ...trigger.eventData, [field.name]: e.target.value }
                                ...
                                </select>) : field.type === 'boolean' ? (<select value={trigger.eventData?.[field.name] || ''} onChange={(e) => setTrigger({
                        ...trigger,
                        eventData: { ...trigger.eventData, [field.name]: e.target.value === 'true' }
                                ...
                                </select>) : (<TextInput value={trigger.eventData?.[field.name] || ''} onChange={(e) => setTrigger({
                        ...trigger,
                        eventData: { ...trigger.eventData, [field.name]: e.target.value }
                    })} placeholder={`Enter ${field.name}`} className="w-full text-sm"/>)}
                        </div>))}
                </div>)}

            <div className="flex items-center space-x-2">
                <input type="checkbox" id="enabled" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="accent-accent"/>
                <label htmlFor="enabled" className="text-sm text-foreground">
                    Enable this rule
                </label>
            </div>
        </div>);
    const renderConditionsStep = () => (<div className="space-y-4">
            ...
            {conditions.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                    <p>No conditions added. Rule will trigger for any matching event.</p>
                </div>) : (<div className="space-y-3">
                    {conditions.map((condition, index) => (<div key={condition.id} className="border rounded p-3 bg-muted/20">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-foreground">
                                    Condition {index + 1}
                                </span>
                                <Button onClick={() => removeCondition(condition.id)} variant="danger" size="sm">
                                    Remove
                                </Button>
                            </div>
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                <select value={condition.type} onChange={(e) => updateCondition(condition.id, {
                    type: e.target.value,
                    field: CONDITION_FIELD_OPTIONS[e.target.value]?.[0]?.field || ''
                                ...
                                <select value={condition.field} onChange={(e) => updateCondition(condition.id, { field: e.target.value })} className="border rounded p-1 bg-background text-foreground text-sm">
                                    {CONDITION_FIELD_OPTIONS[condition.type]?.map(option => (<option key={option.field} value={option.field}>
                                            {option.label}
                                        </option>))}
                                </select>

                                <select value={condition.operator} onChange={(e) => updateCondition(condition.id, { operator: e.target.value })} className="border rounded p-1 bg-background text-foreground text-sm">
                                ...
                                <TextInput value={condition.value} onChange={(e) => updateCondition(condition.id, { value: e.target.value })} placeholder="Value" className="text-sm"/>
                            </div>
                        </div>))}
                </div>)}
        </div>);
    const renderActionsStep = () => (<div className="space-y-4">
            <div className="flex items-center justify-between">
            ...
            {actions.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                    ...
                    {actions.map((action, index) => (<div key={action.id} className="border rounded p-3 bg-muted/20">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-foreground">
                                    {action.type === 'obs' ? '🎛️ OBS' : '🤖 Streamer.bot'} Action {index + 1}
                                </span>
                                <Button onClick={() => removeAction(action.id)} variant="danger" size="sm">
                                    Remove
                                </Button>
                            </div>

                            {action.type === 'obs' ? (<div className="space-y-2">
                                    <select value={action.data.type} onChange={(e) => updateAction(action.id, {
                        data: { type: e.target.value }
                                    ...
                                    {action.data.type === 'setCurrentProgramScene' && (<select value={action.data.sceneName || ''} onChange={(e) => updateAction(action.id, {
                            data: { type: 'setCurrentProgramScene', sceneName: e.target.value }
                                    ...
                                    {action.data.type === 'setSceneItemEnabled' && (<>
                                            <select value={action.data.sceneName || ''} onChange={(e) => {
                            ...
                            updateAction(action.id, {
                                data: {
                                    ...
                                    sceneItemEnabled: currentData.sceneItemEnabled ?? true
                                            ...
                                            <TextInput value={action.data.sourceName || ''} onChange={(e) => {
                            const currentData = action.data;
                            updateAction(action.id, {
                                data: {
                                    type: 'setSceneItemEnabled',
                        ...
                        }} placeholder="Source Name" className="w-full text-sm"/>
                                            <select value={action.data.sceneItemEnabled?.toString() || 'true'} onChange={(e) => {
                            const currentData = action.data;
                            updateAction(action.id, {
                                data: {
                                    type: 'setSceneItemEnabled',
                                    ...
                                    {action.data.type === 'setInputMute' && (<>
                                            <TextInput value={action.data.inputName || ''} onChange={(e) => updateAction(action.id, {
                            data: {
                                type: 'setInputMute',
                                inputName: e.target.value,
                                inputMuted: action.data.inputMuted ?? true
                            }
                        })} placeholder="Input Name" className="w-full text-sm"/>
                                            <select value={action.data.inputMuted?.toString() || 'true'} onChange={(e) => updateAction(action.id, {
                            data: {
                                type: 'setInputMute',
                                inputName: action.data.inputName || '',
                                inputMuted: e.target.value === 'true'
                                    ...
                                    {action.data.type === 'setInputVolume' && (<>
                                            <TextInput value={action.data.inputName || ''} onChange={(e) => updateAction(action.id, {
                            data: {
                                type: 'setInputVolume',
                                inputName: e.target.value,
                                inputVolumeMul: action.data.inputVolumeMul || 1.0
                            }
                        })} placeholder="Input Name" className="w-full text-sm"/>
                                            <TextInput type="number" value={action.data.inputVolumeMul || ''} onChange={(e) => updateAction(action.id, {
                            data: {
                                type: 'setInputVolume',
                                inputName: action.data.inputName || '',
                                inputVolumeMul: parseFloat(e.target.value) || 1.0
                            }
                        })} placeholder="Volume (0.0 to 1.0)" className="w-full text-sm" step="0.1" min="0" max="1"/>
                                        </>)}
                                </div>) : (<div className="space-y-2">
                                    <select value={action.data.actionName || ''} onChange={(e) => updateAction(action.id, {
                        data: { actionName: e.target.value, args: {} }
                    })} className="w-full border rounded p-2 bg-background text-foreground text-sm">
                                        <option value="">Select Streamer.bot Action...</option>
                                        {streamerBotActions.map(sbAction => (<option key={sbAction.id} value={sbAction.name}>
                                                {sbAction.name}
                                            </option>))}
                                    </select>
                                </div>)}
            ...
            <div className="bg-muted/20 rounded p-4 space-y-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        ...
                        {Object.keys(trigger.eventData || {}).length > 0 && (<span> with specific data filters</span>)}
                    </p>
                </div>
                ...
                {conditions.length > 0 && (<div>
                        ...
                        <ul className="text-sm text-muted-foreground ml-4">
                            {conditions.map((condition) => (<li key={condition.id}>
                ...
                <div>
                    ...
                    <ul className="text-sm text-muted-foreground ml-4">
                        {actions.map((action) => (<li key={action.id}>
                                {action.type === 'obs' ? 'OBS' : 'Streamer.bot'}: {action.type === 'obs'
                ? action.data.type
    ...
    return (<Modal isOpen={isOpen} onClose={onClose} title={editingRule ? 'Edit Automation Rule' : 'Create Automation Rule'} size="xl">
            <div className="space-y-6">
                ...
                <div className="flex justify-between pt-4 border-t border-border">
                    <div className="space-x-2">
                        {currentStep !== 'trigger' && (<Button onClick={() => {
                ...
                setCurrentStep(steps[currentIndex - 1]);
                    ...
                    <div className="space-x-2">
                        ...
                        {currentStep !== 'review' ? (<Button onClick={() => {
                const steps = ['trigger', 'conditions', 'actions', 'review'];
                const currentIndex = steps.indexOf(currentStep);
                setCurrentStep(steps[currentIndex + 1]);
            }}>
