...
export const ConnectionForm = ({ onConnect, onDisconnect, isConnected, isConnecting, defaultUrl, error, geminiApiKey, envGeminiApiKey, onGeminiApiKeyChange, isGeminiClientInitialized, geminiInitializationError, streamerBotAddress, setStreamerBotAddress, streamerBotPort, setStreamerBotPort, onStreamerBotConnect, onStreamerBotDisconnect, isStreamerBotConnected = false, isStreamerBotConnecting = false, accentColorName, }) => {
    ...
    const [address, setAddress] = useState(persistedConnectionSettings.obsWebSocketUrl || defaultUrl);
    const [password, setPassword] = useState(persistedConnectionSettings.obsPassword || '');
    const [localGeminiKey, setLocalGeminiKey] = useState(geminiApiKey);
    const [showPasswordField, setShowPasswordField] = useState(Boolean(persistedConnectionSettings.obsPassword));
    const [autoConnect, setAutoConnect] = useState(Boolean(persistedConnectionSettings.autoConnect));
    const storedGeminiApiKey = useAppStore((state) => state.geminiApiKey);
    const [showApiKeyOverride, setShowApiKeyOverride] = useState(Boolean(storedGeminiApiKey) // Start with true if there's already a stored API key
    );
    ...
    const [streamerBotExpanded, setStreamerBotExpanded] = useState(false);
    const obsConnectingDotRef = useRef(null);
    const geminiStatusDotRef = useRef(null);
    ...
    const handleAutoConnectChange = (e) => {
    ...
    return (<div className="space-y-2 max-w-4xl mx-auto p-0">
      ...
      <CollapsibleCard title="OBS Studio Connection" emoji="🎬" isOpen={obsExpanded} onToggle={() => setObsExpanded(!obsExpanded)} accentColor={accentColor} domain="obsproject.com" className="relative group">
        <CardContent className="px-3 pb-3 pt-2">
          ...
          <div className="absolute top-1 right-8 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10 flex gap-1">
            ...
            {/* Status indicator */}
            <span ref={obsConnectingDotRef} className={cn("inline-block w-2 h-2 rounded-full border border-white transition-all duration-300", isConnected
            ? 'bg-primary'
            : isConnecting
                ? 'bg-yellow-500'
                : 'bg-destructive')} title={isConnected ? 'Connected' : isConnecting ? 'Connecting...' : 'Disconnected'}/>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-2 items-end">
              <div className="lg:col-span-2 flex items-center gap-2">
                <TextInput label="WebSocket URL" id="obs-address" type="text" value={address} onChange={handleAddressChange} disabled={isConnected || isConnecting} placeholder="ws://localhost:4455" accentColorName={accentColorName} size="sm"/>
              </div>
              ...
              <div className="flex items-center space-x-2 lg:col-span-1">
                <label className="flex items-center space-x-2 text-xs text-muted-foreground cursor-pointer group">
                  <input type="checkbox" id="enable-password" checked={showPasswordField} onChange={(e) => {
            ...
            setShowPasswordField(e.target.checked);
        }} disabled={isConnected || isConnecting} className="appearance-none h-4 w-4 border-2 border-border rounded-sm bg-background
              ...
              <div className="flex items-center space-x-2 lg:col-span-1">
                <label className="flex items-center space-x-2 text-xs text-muted-foreground cursor-pointer group" title="Automatically connect to OBS when the app loads">
                  <input type="checkbox" id="auto-connect" checked={autoConnect} onChange={handleAutoConnectChange} className="appearance-none h-4 w-4 border-2 border-border rounded-sm bg-background
                               checked:bg-primary checked:border-transparent focus:outline-none 
                               focus:ring-2 focus:ring-offset-0 focus:ring-ring focus:ring-opacity-50
            ...
            {showPasswordField && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                <TextInput label="Password" id="obs-password" name="obs-password" type="password" value={password} onChange={handlePasswordChange} disabled={isConnected || isConnecting} placeholder="Enter OBS WebSocket password" accentColorName={accentColorName} size="sm" autoComplete="current-password"/>
              </div>)}

            {hasAttemptedConnect && error && (<div className="p-2 bg-destructive/10 border border-destructive/30 rounded">
                <p className="text-destructive text-xs font-medium">{error}</p>
              </div>)}

            {/* Connect Button - Only show when not connected */}
            {!isConnected && (<div className="flex justify-start pt-1">
                <Button type="submit" variant="primary" isLoading={isConnecting} disabled={isConnecting} size="sm" accentColorName={accentColorName}>
      ...
      <CollapsibleCard title="Streamer.bot" emoji="🤖" isOpen={streamerBotExpanded} onToggle={() => setStreamerBotExpanded(!streamerBotExpanded)} accentColor={accentColor} domain="streamer.bot" className="relative group">
        <CardContent className="px-3 pb-3 pt-2">
          {/* Connection Status and Action Buttons - positioned absolutely for hover */}
          <div className="absolute top-1 right-8 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10 flex gap-1">
            ...
            {/* Status indicator */}
            <span className={`inline-block w-2 h-2 rounded-full border border-white ${isStreamerBotConnecting
            ? 'bg-yellow-500 animate-pulse'
            : isStreamerBotConnected
                ? 'bg-green-500'
                : 'bg-muted'}`} title={isStreamerBotConnecting
            ? 'Connecting...'
            : isStreamerBotConnected
                ? 'Connected'
                : 'Disconnected'}/>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <TextInput label="Address" id="streamerbot-address" type="text" value={streamerBotAddress || 'localhost'} onChange={(e) => setStreamerBotAddress(e.target.value || 'localhost')} accentColorName={accentColorName} className="text-sm" size="sm"/>
              <TextInput label="Port" id="streamerbot-port" type="text" value={streamerBotPort || '8080'} onChange={(e) => setStreamerBotPort(e.target.value || '8080')} accentColorName={accentColorName} className="text-sm" size="sm"/>
            </div>

            {/* StreamerBot Connection Button */}
            <div className="flex space-x-2">
              {!isStreamerBotConnected ? (<Button onClick={onStreamerBotConnect} disabled={!(streamerBotAddress || 'localhost').trim() || !(streamerBotPort || '8080').trim() || isStreamerBotConnecting} isLoading={isStreamerBotConnecting} size="sm" className="flex-1" accentColorName={accentColorName}>
                  {isStreamerBotConnecting ? 'Connecting...' : 'Connect'}
                </Button>) : (<Button onClick={onStreamerBotDisconnect} variant="secondary" size="sm" className="flex-1" accentColorName={accentColorName}>
                  Disconnect
                </Button>)}
            </div>
      ...
      {/* Gemini AI Section */}
      <CollapsibleCard title="Gemini AI Integration" emoji="🧠" isOpen={geminiExpanded} onToggle={() => setGeminiExpanded(!geminiExpanded)} accentColor={accentColor} domain="gemini.google.com" className="relative group">
        <CardContent className="px-3 pb-3 pt-2 space-y-3">
          ...
          <div className="absolute top-1 right-8 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10 flex gap-1">
            {(isGeminiClientInitialized || geminiInitializationError) && (<>
                ...
                <Tooltip content="Status">
                  <span ref={geminiStatusDotRef} className={cn("inline-block w-3 h-3 rounded-full border border-white transition-all duration-300", geminiInitializationError
                ? 'bg-red-500'
            ...
            {!isGeminiClientInitialized && !geminiInitializationError && (<Tooltip content="Initialize">
          ...
          <div className="p-3 rounded bg-card border border-border">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className={cn("text-sm font-medium", envGeminiApiKey ? 'text-primary' : 'text-destructive')}>
                  {envGeminiApiKey ? 'Found' : 'Not found'}
                </span>
              </div>
            </div>
            {!envGeminiApiKey && (<p className="text-sm text-muted-foreground">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 items-end">
            <div className="flex items-center space-x-2 lg:col-span-3">
              <label className="flex items-center space-x-2 text-sm text-muted-foreground cursor-pointer group">
                <input type="checkbox" id="enable-api-override" checked={showApiKeyOverride} onChange={(e) => handleApiKeyOverrideToggle(e.target.checked)} className="appearance-none h-4 w-4 border-2 border-border rounded-sm bg-background
                             checked:bg-primary checked:border-transparent focus:outline-none 
                             focus:ring-2 focus:ring-offset-0 focus:ring-ring focus:ring-opacity-50
                             transition duration-150 group-hover:border-border"/>
          ...
          {showApiKeyOverride && (<form onSubmit={(e) => { e.preventDefault(); }}>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 items-end">
                ...
                if (navigator.clipboard) {
                  ...
                  {localGeminiKey && (<Button type="button" variant="secondary" size="sm" onClick={() => {
                    setLocalGeminiKey('');
                    onGeminiApiKeyChange('');
                }} accentColorName={accentColorName}>
                      🗑️
          ...
          <div className="text-sm text-muted-foreground bg-card p-3 rounded border border-border">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 underline transition-all">
                Get API Key →
              </a>
            </div>
            {isStorageAvailable() && showApiKeyOverride && (<p className="text-primary mt-2">💾 Manual input auto-saved</p>)}
          </div>
        </CardContent>
      </CollapsibleCard>
    </div>);
