 ...
 */
export function getCustomApiKey(service) {
 ...
 * @param {string} key
 */
export function setCustomApiKey(service, key) {
 ...
 * @param {string} service
 */
export function removeCustomApiKey(service) {
    const keys = getCustomApiKeys();
    delete keys[service];
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(keys));
}
...
const AdvancedPanel = () => {
    ...
    deleteAutomationRule, toggleAutomationRule, handleObsAction, 
    // setStreamerBotServiceInstance,
    getLogFiles, uploadLog } = useAppStore((state) => state.actions);
    ...
    const [ruleBuilderInitialEvent, setRuleBuilderInitialEvent] = useState();
    ...
    const [obsEventSubscriptions, setObsEventSubscriptions] = useState(() => {
        try {
    ...
    const handleObsEventToggle = (eventName) => {
        setObsEventSubscriptions((prev) => prev.includes(eventName)
    ...
    useEffect(() => {
        if (obsServiceInstance || streamerBotServiceInstance) {
            automationService.initialize(automationRules, streamerBotServiceInstance, handleObsAction, addMessage);
        }
    ...
    const handleCreateRule = (eventName) => {
    ...
    return (<div className="space-y-2 max-w-2xl mx-auto p-0">
            {/* Memory Context Section */}
            <CollapsibleCard title="Memory Context" emoji="🧠" isOpen={openMemory} onToggle={() => setOpenMemory((v) => !v)} accentColor={accentColor}>
                <CardContent className="px-3 pb-3">
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2">
                            <input className="w-full border rounded p-2 bg-background" value={userInput} onChange={(e) => setUserInput(e.target.value)} placeholder="Enter stream/community details"/>
                            <Button onClick={() => {
            if (userInput.trim()) {
                addToUserDefinedContext(userInput.trim());
                setUserInput('');
            }
        }} disabled={!userInput.trim()} size="sm">
                                Add
                            </Button>
                        </div>
                        {userDefinedContext.length > 0 && (<div className="space-y-2">
                                <label className="block text-sm font-medium text-primary">Current Context:</label>
                                <div className="space-y-1 max-h-32 overflow-y-auto">
                                    {userDefinedContext.map((context, index) => (<div key={index} className="flex items-center justify-between bg-muted/30 px-3 py-2 rounded text-sm">
                                            <span className="text-foreground flex-1 mr-2">{context}</span>
                                            <Button onClick={() => removeFromUserDefinedContext(context)} variant="danger" size="sm">
                                                Remove
                                            </Button>
                                        </div>))}
                                </div>
            ...
            <CollapsibleCard title="Automation Rules" emoji="🤖" isOpen={openAutomation} onToggle={() => setOpenAutomation((v) => !v)} accentColor={accentColor}>
                <CardContent className="px-3 pb-3">
                    <div className="space-y-4">
                        ...
                        {automationStats.totalTriggers > 0 && (<div className="text-xs text-muted-foreground">
                                📊 Total triggers: {automationStats.totalTriggers}
                            </div>)}

                        {automationRules.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
                                ...
                                {automationRules.map((rule) => (<div key={rule.id} className="border rounded p-3 bg-muted/20">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center space-x-2">
                                                <span className={cn("w-2 h-2 rounded-full", rule.enabled ? "bg-green-500" : "bg-gray-400")}/>
                                                <span className="text-sm font-medium text-foreground">
                                                    {rule.name}
                                                </span>
                                                {rule.triggerCount && rule.triggerCount > 0 && (<span className="text-xs bg-muted px-2 py-0.5 rounded">
                                                        {rule.triggerCount} triggers
                                                    </span>)}
                                            </div>
                                            <div className="flex items-center space-x-1">
                                                <Tooltip content={rule.enabled ? 'Disable rule' : 'Enable rule'}>
                                                    <button onClick={() => toggleAutomationRule(rule.id)} className={cn("w-6 h-6 rounded text-xs transition-colors", rule.enabled
                    ? "bg-green-100 text-green-700 hover:bg-green-200"
                    : "bg-gray-100 text-gray-500 hover:bg-gray-200")}>
                                                        {rule.enabled ? '✓' : '○'}
                                                    </button>
                                                </Tooltip>
                                                <Button onClick={() => handleEditRule(rule)} size="sm" variant="secondary">
                                                    Edit
                                                </Button>
                                                <Button onClick={() => handleDeleteRule(rule.id)} size="sm" variant="danger">
                                                    Delete
                                                </Button>
                                            </div>
                                        </div>
                                        <div className="text-xs text-muted-foreground">
                                            <div className="mb-1">
                                                ...
                                                {Object.keys(rule.trigger.eventData || {}).length > 0 && (<span> (with filters)</span>)}
                                            </div>
                                            {rule.conditions && rule.conditions.length > 0 && (<div className="mb-1">
                                                    <strong>Conditions:</strong> {rule.conditions.length} condition(s)
                                                </div>)}
                                            <div>
                                                <strong>Actions:</strong> {rule.actions.length} action(s)
                                                {rule.actions.map((action, index) => (<span key={index} className="ml-1">
                                                        {action.type === 'obs' ? '🎛️' : '🤖'}
                                                    </span>))}
                                            </div>
                                            {rule.lastTriggered && (<div className="mt-1 text-xs opacity-75">
            ...
            <CollapsibleCard title="Audio Output Routing" emoji="🔊" isOpen={openAudio} onToggle={() => setOpenAudio((v) => !v)} accentColor={accentColor}>
                <CardContent className="px-3 pb-3">
                    <div className="space-y-4">
                        <AudioOutputSelector />
                    </div>
            ...
            <CollapsibleCard title="OBS Event Subscriptions" emoji="🎛️" isOpen={openObsEvents} onToggle={() => setOpenObsEvents((v) => !v)} accentColor={accentColor}>
                <CardContent className="px-3 pb-3">
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1">
                            {OBS_EVENT_LIST.map((event) => (<div key={event.name} className="flex items-center gap-2 py-1">
                                    <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                                        <input type="checkbox" checked={obsEventSubscriptions.includes(event.name)} onChange={() => handleObsEventToggle(event.name)} className="accent-accent h-4 w-4 rounded border border-border"/>
                                    ...
                                    <AddToContextButton contextText={`OBS Event: '${event.name}'\n\nExample: \`\`\`json\n${JSON.stringify({ event: event.name, example: '...event data here...' }, null, 2)}\n\`\`\``}/>
                                    <Tooltip content={`Create automation rule for ${event.name}`}>
                                        <Button onClick={() => handleCreateRule(event.name)} size="sm" variant="secondary" className="ml-1 text-xs px-2 py-1">
                                            🤖
                                        </Button>
                                    </Tooltip>
                                </div>))}
            ...
            {/* OBS Logs Section */}
            <CollapsibleCard title="OBS Logs" emoji="📝" isOpen={openLogs} onToggle={() => setOpenLogs((v) => !v)} accentColor={accentColor}>
                <CardContent className="px-3 pb-3">
                    <div className="space-y-4">
                        <p className="text-sm text-muted-foreground">
            ...
            try {
        ...
        }} size="sm" disabled={isUploadingLog}>
                                {isUploadingLog ? 'Uploading...' : 'Upload Latest Log'}
                            </Button>
                        </div>

                        {uploadResult && (<div className={`p-3 rounded text-sm ${uploadResult.success
                ...
                : 'bg-red-100 text-red-800 border border-red-200'}`}>
                                ...
                                {uploadResult.success && uploadResult.url && (<div className="mt-2">
                                        <a href={uploadResult.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline text-xs break-all">
                                            {uploadResult.url}
                                        </a>
                                    </div>)}
                            </div>)}

                        {obsLogFiles && obsLogFiles.length > 0 && (<div className="space-y-2">
                                <label className="block text-sm font-medium text-primary">Available Log Files:</label>
                                <div className="max-h-40 overflow-y-auto">
                                    {obsLogFiles.map((logFile, index) => (<div key={index} className="flex items-center justify-between bg-muted/30 px-3 py-2 rounded text-sm">
                                            <div>
                                                <div className="font-medium text-foreground">{logFile.fileName || `Log ${index + 1}`}</div>
                                                {logFile.fileSize && (<div className="text-xs text-muted-foreground">
                                                        Size: {(logFile.fileSize / 1024).toFixed(1)} KB
                                                    </div>)}
                                                {logFile.creationTime && (<div className="text-xs text-muted-foreground">
                                                        Created: {new Date(logFile.creationTime).toLocaleString()}
                                                    </div>)}
                                            </div>
                                            <AddToContextButton contextText={`OBS Log File: ${logFile.fileName || `Log ${index + 1}`}${logFile.fileSize ? ` (${(logFile.fileSize / 1024).toFixed(1)} KB)` : ''}${logFile.creationTime ? ` - Created: ${new Date(logFile.creationTime).toLocaleString()}` : ''}`} title="Add log file info to chat context"/>
                                        </div>))}
                                </div>
                            </div>)}

                        {obsLogFiles && obsLogFiles.length === 0 && (<div className="text-center text-muted-foreground py-4">
            ...
            <CollapsibleCard title="Custom API Keys" emoji="🔑" isOpen={openApiKeys} onToggle={() => setOpenApiKeys((v) => !v)} accentColor={accentColor}>
                <CardContent className="px-3 pb-3">
                    <div className="space-y-4">
                        <p className="text-sm text-muted-foreground">
                        ...
                        <div className="space-y-3">
                            {API_KEY_SERVICES.map(({ id, label }) => (<div key={id} className="flex flex-col gap-1">
                                    <label className="text-sm font-medium text-primary">{label} API Key</label>
                                    <div className="flex gap-2 items-center">
                                        <input className="w-full border rounded p-2 bg-background" type="password" value={apiKeyInputs[id] || ''} onChange={e => handleApiKeyChange(id, e.target.value)} placeholder={`Enter your ${label} API key`} autoComplete="new-password"/>
                                        <Button onClick={() => handleApiKeySave(id)} size="sm" disabled={!(apiKeyInputs[id] && apiKeyInputs[id].trim())}>
                                            {apiKeys[id] ? 'Update' : 'Save'}
                                        </Button>
                                        {apiKeys[id] && (<Button onClick={() => handleApiKeyRemove(id)} variant="secondary" size="sm">
                                                Remove
                                            </Button>)}
                                    </div>
                                    {apiKeys[id] && (<div className="text-xs text-muted-foreground break-all">
            ...
            {/* Reset Section */}
            <CollapsibleCard title="Reset Settings" emoji="🔄" isOpen={openReset} onToggle={() => setOpenReset((v) => !v)} accentColor={accentColor}>
                <CardContent className="px-3 pb-3">
                    <div className="space-y-4">
                        <p className="text-sm text-muted-foreground">
                        ...
                        <Button onClick={handleResetAllSettings} variant="danger" size="sm">
                            Reset All Settings
                        </Button>
                    </div>
                </CardContent>
            ...
            <AutomationRuleBuilder isOpen={showRuleBuilder} onClose={handleCloseRuleBuilder} initialEventName={ruleBuilderInitialEvent} editingRule={editingRule}/>
        </div>);
};
