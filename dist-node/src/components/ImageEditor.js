...
export const ImageEditor = () => {
    ...
    return (<div className="max-w-2xl mx-auto rounded-xl border border-ctp-surface1 bg-ctp-base shadow-lg">
            ...
            <div className="flex flex-row flex-wrap gap-2 items-center mb-4 px-4 w-full">
                <div className="flex-1 min-w-[180px]">
                    <input type="file" accept="image/*" ref={fileInputRef} onChange={handleFileChange} className="w-full px-2 py-2 rounded bg-ctp-surface0 border border-ctp-surface2 text-ctp-text focus:ring-ctp-mauve focus:border-ctp-mauve transition-all duration-150"/>
                </div>
                <div className="flex-shrink-0">
                    <button className="glass-button px-4 py-2 bg-ctp-mauve text-ctp-base rounded shadow hover:bg-ctp-mauve/80 transition w-full" onClick={handleRemoveBackground} disabled={!inputUrl || loading}>
            ...
            <div className="flex flex-row gap-2 items-stretch justify-center mb-4 w-full px-2">
                    ...
                    {inputUrl ? (<>
                            <div className="flex items-center justify-center w-full h-full">
                                <Tooltip content="Click to preview">
                                    <img src={inputUrl} alt="Input" className="w-full h-full max-h-52 rounded shadow cursor-pointer object-contain bg-ctp-mantle" onClick={() => setInputModalOpen(true)} style={{ aspectRatio: "1/1" }}/>
                                </Tooltip>
                            </div>
                ...
                <div className="flex-[2] flex flex-col items-center min-w-0 w-2/3 bg-ctp-surface0 rounded-lg p-2 border border-ctp-mauve/40 justify-center">
                    {outputUrl ? (<>
                            <div className="flex items-center justify-center w-full h-full">
                                <Tooltip content="Click to preview">
                                    <img src={outputUrl} alt="Output" className="w-full h-full max-h-64 rounded shadow cursor-pointer object-contain border-2 border-ctp-mauve bg-ctp-mantle" onClick={() => setOutputModalOpen(true)} style={{ aspectRatio: "1/1.1" }}/>
                                </Tooltip>
                            </div>
                            <span className="text-xs text-ctp-mauve mt-1 font-semibold">Result</span>
                            <a href={outputUrl} download="background-removed.png" className="glass-button px-3 py-1 mt-2 inline-block bg-ctp-mauve text-ctp-base rounded shadow hover:bg-ctp-mauve/80 transition">
                                Download PNG
                            </a>
                        </>) : (<div className="w-full h-32 flex items-center justify-center bg-ctp-surface1 rounded text-xs text-ctp-subtext1">
                            No result yet
            ...
            {inputModalOpen && inputUrl && (<Modal isOpen={inputModalOpen} onClose={() => setInputModalOpen(false)} title="Input Image Preview" size="lg">
            ...
            {outputModalOpen && outputUrl && (<Modal isOpen={outputModalOpen} onClose={() => setOutputModalOpen(false)} title="Background Removed Preview" size="lg" actions={[
                {
                    label: "Download",
                    onClick: () => {
