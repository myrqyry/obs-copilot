...
export const GeminiChat = ({ geminiApiKeyFromInput, streamerBotService, onRefreshData, setErrorMessage, chatInputValue, onChatInputChange, accentColorName, messages, onAddMessage, isGeminiClientInitialized, 
// geminiInitializationError,
onSetIsGeminiClientInitialized, onSetGeminiInitializationError, onStreamerBotAction, }) => {
    // Merge all needed state into a single useAppStore call for efficiency
    const { userSettings: { bubbleFillOpacity, backgroundOpacity, extraDarkMode, customChatBackground, autoApplySuggestions, flipSides, }, 
    /* Removed chatBackgroundBlendMode as it does not exist in AppState */
    scenes, currentProgramScene, sources, streamStatus, recordStatus, videoSettings, actions, isConnected, userDefinedContext, } = useAppStore();
    ...
    const handleAddToContext = useCallback((text) => {
...
`;
    ...
    const handleSend = async () => {
        if (!chatInputValue.trim() || !ai.current || isLoading)
            return;
        // Trigger text split animation before sending
        const inputElement = chatInputRef.current;
        if (inputElement) {
            // Find the send button element by class or nearby element
            const sendButton = inputElement.parentElement?.querySelector('button[class*="primary"]');
    ...
    const handleRegenerate = async (messageId) => {
        ...
        if (messageToRegenerate.role !== 'model')
            return;
    ...
    return (<div className="flex flex-col h-full bg-background border-l border-r border-b border-border rounded-b-lg shadow-lg relative">
      ...
      {validatedCustomChatBackground !== 'none' && (<>
          <div className="absolute inset-0 rounded-b-lg" style={{
                ...
                zIndex: 0,
                mixBlendMode: 'normal', // Removed chatBackgroundBlendMode reference
            }}></div>
        </>)}
      <div className="flex-grow p-2 space-y-2 overflow-y-auto relative z-10">
        {messages.map((msg, idx) => (<motion.div key={msg.id || idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            <ChatMessageItem message={msg} onSuggestionClick={handleSuggestionClick} accentColorName={accentColorName} obsSources={msg.type === "source-prompt" ? obsData.sources : undefined} onAddToContext={handleAddToContext} extraDarkMode={extraDarkMode} flipSides={flipSides} showSuggestions={msg.showSuggestions || false} onRegenerate={handleRegenerate} userChatBubbleColorName="mauve" // Default CatppuccinAccentColorName
      ...
      <div className="p-3 border-t border-border bg-background rounded-b-lg">
        ...
        {isGeminiInitializing && (<div className="flex items-center justify-center mb-2">
        ...
        <div className="flex items-center space-x-2 relative">
          ...
          {hasContext && (<div className="absolute left-0 -top-7 flex items-center space-x-1 z-20">
          ...
          <div className="relative flex-grow">
            ...
            {streamerBotActions.length > 0 && (<div className="absolute right-0 top-0 z-20 flex flex-col items-end space-y-1 pr-2 pt-1">
                <div className="text-xs text-muted-foreground mb-1">Streamer.bot Actions:</div>
                {streamerBotActions.slice(0, 3).map((action) => (<button key={action.id} className="px-2 py-1 rounded-lg bg-[rgba(80,90,255,0.10)] text-indigo-400 border border-indigo-300 text-xs mb-1 hover:bg-indigo-200/30 hover:text-indigo-600 transition-colors duration-150 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400/40" onClick={() => handleSuggestionClick(`Trigger Streamer.bot action: ${action.name}`)} style={{
            ...
            <div className="flex items-center">
              <input ref={chatInputRef} id="gemini-input" type="text" value={chatInputValue} onChange={(e) => onChatInputChange(e.target.value)} onKeyDown={(e) => {
            if (e.key === 'Enter' && !isLoading && isGeminiClientInitialized && chatInputValue.trim()) {
                handleSend();
            }
        }} className="w-full pl-14 pr-3 py-2 rounded-xl bg-[rgba(34,37,51,0.92)] border border-[rgba(80,80,120,0.25)] text-[var(--text-color)] placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-accent focus:border-accent transition-all duration-200 shadow-lg backdrop-blur-md disabled:opacity-60 disabled:cursor-not-allowed" placeholder={isLoading ? 'Waiting for Gemini...' : 'Type your message...'} disabled={isLoading || !isGeminiClientInitialized} autoComplete="off" spellCheck={true} style={{
            ...
            boxShadow: '0 2px 12px 0 rgba(30,30,60,0.10)',
              ...
              <Button type="button" variant="primary" size="sm" accentColorName={accentColorName} onClick={handleSend} disabled={isLoading || !chatInputValue.trim()} className="ml-2 flex items-center justify-center">
          ...
          <div className="absolute left-3 top-1/2 transform -translate-y-1/2 z-10 flex items-center space-x-1">
            <Tooltip content={useGoogleSearch ? "Web search enabled" : "Click to enable web search"}>
              <button onClick={() => setUseGoogleSearch(!useGoogleSearch)} disabled={!isGeminiClientInitialized} className={`p-1 rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-primary/50 ${useGoogleSearch
            ? 'text-primary bg-primary/10 hover:bg-primary/20'
            : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'} ${!isGeminiClientInitialized ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`} aria-label={useGoogleSearch ? "Disable web search" : "Enable web search"}>
        ...
        }} disabled={!isGeminiClientInitialized || !isConnected || !currentProgramScene} className={`p-1 rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-orange-500/50 ${isConnected && currentProgramScene
            ? 'text-orange-500 hover:text-orange-600 hover:bg-orange-500/10'
            : 'text-muted-foreground cursor-not-allowed opacity-50'}`} aria-label="Take screenshot of current scene">
