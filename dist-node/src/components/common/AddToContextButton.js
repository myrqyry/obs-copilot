 ...
 */
const AddToContextButton = ({ contextText, disabled, title, className }) => {
    ...
    const onSendToGeminiContext = useAppStore((state) => state.onSendToGeminiContext) || ((text) => addMessage({ role: 'system', text }));
    return (<Tooltip content={title || 'Add to chat context'}>
            <button type="button" className={`ml-1 p-1 rounded hover:bg-accent/20 transition-colors focus:outline-none focus:ring-2 focus:ring-accent/50 ${className || ''}`} disabled={disabled} onClick={() => onSendToGeminiContext(contextText)} aria-label={title || 'Add to chat context'}>
