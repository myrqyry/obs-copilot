...
export const MiniPlayerIcons = {
    ...
    Pause: () => (<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>),
    Stop: () => (<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h12v12H6z"/></svg>),
    Minimize: () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="11" width="16" height="2" rx="1" fill="currentColor"/></svg>),
};
const MiniPlayer = () => {
    ...
    const minimizedNoteRef = useRef(null);
    ...
    if (minimized) {
        return (<>
                ...
                <div className="fixed top-2 right-2 z-[1000]" style={minimized ? { visibility: 'visible', pointerEvents: 'auto' } : { visibility: 'hidden', pointerEvents: 'none', position: 'absolute' }}>
                    <button ref={minimizedNoteRef} aria-label="Show music player" className="w-7 h-7 flex items-center justify-center rounded-full bg-ctp-mauve/80 border border-ctp-mauve/40 shadow hover:bg-ctp-mauve/90 transition-all p-0" onClick={handleExpand} style={{ minWidth: 0, minHeight: 0 }}>
    ...
    return (<>
            ...
            <div className={`fixed top-2 right-2 z-[1000] bg-ctp-base/80 border border-ctp-mauve/30 shadow rounded-lg flex items-center gap-2 px-2 py-0.5 min-w-[90px] max-w-xs group${attention ? ' animate-glow' : ''}`} style={minimized ? { visibility: 'hidden', pointerEvents: 'none', position: 'absolute', backdropFilter: 'blur(6px)', height: '28px' } : { visibility: 'visible', pointerEvents: 'auto', backdropFilter: 'blur(6px)', height: '28px' }}>
                ...
                {activeAudioSource.type === 'music' ? (editing ? (<form onSubmit={e => {
                e.preventDefault();
                setMusicPrompt(editValue);
                setActiveAudioSource({ ...activeAudioSource, prompt: editValue });
                setEditing(false);
            }} className="flex items-center gap-1">
                            <input ref={inputRef} className="text-[10px] px-1 py-0.5 rounded bg-ctp-surface1 border border-ctp-mauve/40 text-ctp-text w-[70px] focus:outline-none focus:ring-1 focus:ring-ctp-mauve" value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={() => setEditing(false)} autoFocus/>
                            <button type="submit" className="text-ctp-mauve hover:text-ctp-green text-xs px-1">✔</button>
                        </form>) : (<div className="relative flex items-center group/music-prompt">
                            <div className="overflow-hidden whitespace-nowrap max-w-[60px]" style={{ position: 'relative' }}>
                ...
                minWidth: '100%',
                            ...
                            <button className="ml-1 opacity-0 group-hover/music-prompt:opacity-100 group-hover:opacity-100 transition-opacity text-ctp-mauve hover:text-ctp-green text-xs px-1" style={{ position: 'absolute', right: -18, top: 0 }} title="Edit prompt" onClick={() => {
        ...
        {activeAudioSource.type === 'tts' && activeAudioSource.url && (<audio ref={audioRef} controls autoPlay onEnded={() => actions.stopMusic()} className="w-64 h-8">
        Your browser does not support the audio element.
    </audio>)}
                {activeAudioSource.type === 'tts' && activeAudioSource.url && (<audio src={activeAudioSource.url} controls autoPlay className="w-20 h-6 ml-1" style={{ minWidth: 60 }}/>)}
                {activeAudioSource.type === 'music' && (<div className="flex items-center gap-1 ml-1">
                ...
                <button aria-label="Minimize music player" className="ml-1 p-0.5 rounded-full hover:bg-ctp-mauve/10 transition-all" onClick={handleMinimize} style={{ alignSelf: 'center' }} ref={minimizedNoteRef}>
