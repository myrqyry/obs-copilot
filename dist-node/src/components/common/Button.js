...
export const Button = ({ children = null, variant = 'primary', size = 'md', isLoading = false, className = '', accentColorName, withAnimation = true, icon, iconPosition = 'left', loadingText = 'Loading...', fullWidth = false, rounded = false, glow = false, ...props }) => {
    ...
    const renderIcon = () => {
        if (!icon)
            return null;
        return (<span className={cn("inline-flex items-center transition-opacity duration-200", iconPosition === 'left' ? 'mr-2' : 'ml-2', isLoading && 'opacity-0')}>
        {icon}
      </span>);
    };
    const renderLoadingSpinner = () => (<svg className="animate-spin h-4 w-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
    </svg>);
    return (<motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} ref={buttonRef} className={cn(baseStyles, getVariantStyles(), sizeStyles[size], borderRadiusStyles, widthStyles, glowStyles, className)} disabled={isLoading || props.disabled} {...props}>
      {isLoading && renderLoadingSpinner()}
      {isLoading && loadingText ? (<span className="transition-opacity duration-200">{loadingText}</span>) : (<>
