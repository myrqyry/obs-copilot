...
export const ChatMessageItem = ({ message, onSuggestionClick, accentColorName: _, obsSources, onSourceSelect, flipSides, extraDarkMode, showSuggestions = false, onAddToContext, onRegenerate, userChatBubbleColorName, modelChatBubbleColorName, customChatBackground, }) => {
    ...
    useLayoutEffect(() => {
        if (itemRef.current) {
            gsap.fromTo(itemRef.current, { opacity: 0, y: 15, scale: 0.98 }, { opacity: 1, y: 0, scale: 1, duration: 0.35, ease: 'power2.out' });
    ...
    const handleBubbleScroll = (event) => {
        ...
        clearTimeout(handleBubbleScroll._scrollTimeout);
        handleBubbleScroll._scrollTimeout = setTimeout(() => setIsScrolling(false), 1000);
    };
    ...
    const bubbleStyle = {
        ...
        mixBlendMode: (chatBubbleBlendMode || 'normal'),
    };
    return (<div ref={itemRef} className={`flex ${containerClasses} mb-3 font-sans ${isSystem ? 'px-6' : isUser ? 'pl-4' : 'pr-4'}`}>
            <div className={`chat-message rounded-2xl shadow-xl border relative font-sans group ${glassEffectClass}
          ${isSystem
        ...
        `} style={bubbleStyle}>
            ...
            color: textColor
                    ...
                    <div ref={bubbleRef} className={`chat-scrollable-content
              ${isShrunk ? 'max-h-80 overflow-y-auto custom-scrollbar shrunk' : ''}
              ${isScrolling ? 'scrolling' : ''}
            `} onScroll={isShrunk ? handleBubbleScroll : undefined}>
                        {message.type === "source-prompt" && obsSources && onSourceSelect ? (<div className="source-selection-container">
                                ...
                                <div className="source-selection-grid grid grid-cols-2 gap-2">
                                    {obsSources.map((source) => (<Tooltip key={source.sourceName} content={source.typeName || source.inputKind || 'Source'}>
                                            <button onClick={() => onSourceSelect(source.sourceName)} className="source-select-btn group flex items-center px-3 py-1.5 bg-background/80 text-foreground border border-border rounded transition-all duration-200 hover:bg-primary hover:text-primary-foreground hover:border-primary focus:outline-none focus:ring-1 focus:ring-primary shadow-sm" tabIndex={0} aria-label={`Select source ${source.sourceName}`}>
                    ...
                    source.inputKind === 'image_source' ? '🖼️' :
                        source.inputKind === 'browser_source' ? '🌐' :
                            source.inputKind === 'window_capture' ? '🪟' :
                                ...
                                {showSuggestions && isSystem && onSuggestionClick && (<div className="mt-3 pt-3 border-t border-opacity-30" style={{ borderColor: bubbleColorHex }}>
                                        <div className="text-sm opacity-90 mb-3 font-normal font-sans"><span className="emoji">✨</span> Try these commands:</div>
                                        <div className="grid grid-cols-2 gap-2">
                                            {memoizedSuggestions.map((suggestion) => (<button key={suggestion.id} onClick={() => onSuggestionClick(suggestion.prompt)} className="text-xs px-2 py-1.5 bg-muted/50 hover:bg-primary/20 text-foreground hover:text-primary-foreground rounded border border-border hover:border-primary transition-all duration-200 text-left group shadow-sm">
                                ...
                                {message.sources && message.sources.length > 0 && (<div className="mt-3 pt-3 border-t border-border">
                                        ...
                                        <div className="space-y-1">
                                            {message.sources.map((source, idx) => (<div key={idx} className="text-xs">
                                                    <a href={source.web?.uri} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 hover:underline transition-colors duration-200">
                                ...
                                {message.type === "choice-prompt" && message.choices && onSuggestionClick && (<div className="mt-3 pt-3 border-t border-opacity-30" style={{ borderColor: bubbleColorHex }}>
                                ...
                                message.choiceType === 'text-source' ? 'Select a text source:' :
                                    message.choiceType === 'screen-source' ? 'Select a screen capture:' :
                                        ...
                                        <div className={`grid gap-2 ${message.choices.length > 4 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                                            {message.choices.map((choice, idx) => (<button key={idx} onClick={() => {
                        if (onAddToContext) {
                    ...
                    {isShrunk && isScrolledFromTop && (<div className="bubble-fade-top"/>)}
                    {/* Absolutely positioned fade overlay, only when shrunk and not at bottom */}
                    {isShrunk && !isScrolledToBottom && (<div className={`bubble-fade-bottom ${isScrolling ? 'opacity-30' : 'opacity-100'}`}/>)}
                </div>

                {/* Timestamp outside of scrollable area */}
                <div className="text-xs mt-1.5 relative z-20 tracking-wider" style={{
            ...
            fontStyle: 'normal'
                ...
                <div className="absolute top-2 right-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex space-x-1">
                    ...
                    {isAssistant && onRegenerate && (<Tooltip content="Regenerate response">
                            <button onClick={handleRegenerate} className="bg-card/90 backdrop-blur-sm text-muted-foreground hover:text-green-500 hover:bg-green-500/10 p-1 rounded-full shadow-md border border-border transition-all duration-200 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-green-500/50" aria-label="Regenerate message">
                                <ArrowPathIcon className="w-3 h-3"/>
                            </button>
                        </Tooltip>)}

                    <Tooltip content="Copy text">
                        <div role="button" onClick={handleCopyText} className="bg-card/90 backdrop-blur-sm text-muted-foreground hover:text-blue-500 hover:bg-blue-500/10 p-1 rounded-full shadow-md border border-border transition-all duration-200 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-blue-500/50" aria-label="Copy message text">
                            <ClipboardDocumentIcon className="w-3 h-3"/>
                        </div>
                    </Tooltip>

                    {onAddToContext && (<Tooltip content="Add to context">
                            <button onClick={handleAddToContextLocal} className="bg-card/90 backdrop-blur-sm text-accent hover:bg-accent/20 p-1 rounded-full shadow-md border border-border transition-all duration-200 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-accent/50" aria-label="Add message to context">
                ...
                {isShrunk && !forceExpand && (<Tooltip content="Expand bubble">
                        <button className="absolute right-3 bottom-3 z-40 bg-card/90 backdrop-blur-sm text-primary hover:text-primary/80 hover:bg-primary/10 p-2 rounded-full shadow-xl border border-border transition-all duration-200 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-primary/50" onClick={() => setForceExpand(true)} aria-label="Expand chat bubble">
                            <ChevronDownIcon className="w-5 h-5"/>
                        </button>
                    </Tooltip>)}
                {forceExpand && (<Tooltip content="Shrink bubble">
                        <button className="absolute right-3 bottom-3 z-40 bg-card/90 backdrop-blur-sm text-muted-foreground hover:text-primary hover:bg-primary/10 p-2 rounded-full shadow-xl border border-border transition-all duration-200 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-primary/50" onClick={() => setForceExpand(false)} aria-label="Shrink chat bubble">
