...
const StreamingAssetsTab = React.memo(() => {
    ...
    const handleBackgroundSearch = useCallback(async (e) => {
        e.preventDefault();
        if (!backgroundQuery.trim())
            return;
        ...
        try {
            ...
            if (backgroundApi === 'wallhaven') {
                ...
                setBackgroundResults(result.results.map((item) => ({
                    id: item.id,
                ...
                setBackgroundResults(data.photos?.map((item) => ({
                    id: item.id,
                    title: item.alt || '',
                    url: item.src.large,
                ...
                setBackgroundResults(data.hits?.map((item) => ({
                    id: item.id,
                    title: item.tags || '',
                ...
                setBackgroundResults(data.results?.map((item) => ({
                    id: item.deviationid,
                    title: item.title || '',
                ...
                setBackgroundResults(data.data?.map((item) => ({
                    id: item.id,
                    title: item.title || '',
    ...
    const handleServiceSwitch = (newService) => {
    ...
    const handleGifSearch = useCallback(async (e) => {
        e.preventDefault();
        if (!gifQuery.trim())
            return;
        setGifLoading(true);
        ...
        try {
            ...
            if (gifApi === 'giphy') {
                ...
                setGifResults(response.data.map((gif) => {
                    const result = {
                        id: gif.id,
                        title: gif.title,
                        images: {
    ...
    const handleShowCategories = async () => {
        ...
        try {
            if (gifApi === 'giphy') {
                ...
                setGifCategories(response.data.map((cat) => ({
                    name: cat.name,
                ...
                const data = await res.json();
                setGifCategories((data.tags || []).map((tag) => ({
                    name: tag.searchterm,
    ...
    const handleSvgSearch = useCallback(async (e) => {
        e.preventDefault();
        if (!svgQuery.trim())
            return;
        setSvgLoading(true);
        ...
        try {
            ...
            if (svgApi === 'iconfinder') {
                ...
                const res = await fetch(url);
                if (!res.ok)
                    throw new Error('Iconfinder API error');
                const data = await res.json();
                ...
                const icons = (data.icons || []).map((icon) => {
                    ...
                    if (icon.vector_sizes && icon.vector_sizes.length > 0) {
                        ...
                        if (firstVector.formats && firstVector.formats.length > 0) {
                            const svgFormat = firstVector.formats.find((f) => f.format === 'svg');
                    ...
                    return { ...icon, svg_url };
                }).filter((icon) => icon.is_premium === false && icon.svg_url);
                setSvgResults(icons.slice(0, limit).map((icon) => ({
                ...
                if (data.icons && data.icons.length > 0) {
                    const iconNames = data.icons.map((icon) => typeof icon === 'string' ? icon : icon.name).slice(0, limit);
                    const svgFetches = iconNames.map(async (iconName) => {
                    ...
                    setSvgResults((await Promise.all(svgFetches)).filter(r => r));
                }
            }
        }
        catch (err) {
    ...
    const handleEmojiSearch = useCallback(async (e) => {
        e.preventDefault();
        if (!emojiQuery.trim())
            return;
        ...
        try {
            ...
            if (emojiApi === 'emojihub') {
                ...
                if (res.ok) {
                    results = (await res.json()).filter((emoji) => emoji.name.toLowerCase().includes(query) ||
    ...
    const handleAddAsBrowserSource = async (url, sourceName) => {
        if (!obsServiceInstance || !isConnected || !currentProgramScene)
            return showFeedback('OBS not connected.');
        try {
    ...
    const handleAddAsImageSource = async (url, sourceName) => {
        if (!obsServiceInstance || !isConnected || !currentProgramScene)
            return showFeedback('OBS not connected.');
        try {
    ...
    const handleAddAsMediaSource = async (url, sourceName) => {
        if (!obsServiceInstance || !isConnected || !currentProgramScene)
            return showFeedback('OBS not connected.');
        try {
    ...
    const handleAddSvgAsBrowserSource = async (svg, sourceName) => {
        if (!obsServiceInstance || !isConnected || !currentProgramScene)
            return showFeedback('OBS not connected.');
        try {
    ...
    const handleAddEmojiAsBrowserSource = async (emoji, sourceName) => {
        if (!obsServiceInstance || !isConnected || !currentProgramScene)
            return showFeedback('OBS not connected.');
        try {
    ...
    const getEmojiChar = (emojiData) => {
        if (!emojiData)
            return '❓';
        if (typeof emojiData === 'string')
            return emojiData;
        if (emojiData.character)
    ...
    const getModalActions = (type, data) => {
        switch (type) {
            case 'sticker':
                return [
    ...
    const renderBackgroundModal = (data) => {
        ...
        if (imageUrl) {
            ...
            return (<div className="p-4 bg-slate-800 rounded-md flex justify-center items-center aspect-square max-w-xs mx-auto">
                    <p className="text-muted-foreground">Image not available</p>
                </div>);
        }
    };
    const renderCollapsibleCard = (cardKey, title, emoji, content, domain, customSvg) => (<Card className={`glass-card shadow rounded-lg transition-all duration-200 p-0 ${openCards[cardKey] ? 'ring-2 ring-accent/40 scale-[1.01]' : 'hover:scale-[1.01] hover:shadow-md'}`}>
            <button className="w-full flex items-center gap-2 px-2 py-0 min-h-0 bg-transparent rounded-t-lg focus:outline-none group" onClick={() => toggleCard(cardKey)} aria-expanded={openCards[cardKey]}>
            ...
            {openCards[cardKey] && (<CardContent className="px-1 pb-1 pt-0 animate-fade-in">
                    {content}
                </CardContent>)}
        </Card>);
    // Enhanced search helpers
    const getSearchSuggestions = () => {
        return SEARCH_SUGGESTIONS[gifApi] || [];
    };
    ...
    const handleGifPageChange = async (newPage) => {
        if (!gifQuery.trim() || gifLoading)
            return;
        setGifLoading(true);
        setGifPage(newPage);
        setSearchError(null);
    ...
    const GridItem = useMemo(() => React.memo(({ item, type, onClick }) => {
        ...
        return (<div className={`gif-grid-item cursor-pointer rounded-lg overflow-hidden transition-all duration-200 hover:scale-105 hover:shadow-lg bg-card border border-border ${item.type === 'stickers' ? 'bg-transparent' : ''}`} data-type={item.type || type} onClick={onClick}>
                {(type === 'gif' || item.type === 'gifs' || item.type === 'stickers') && imageUrl && (<img src={imageUrl} alt={item.title || 'GIF'} className={`w-full h-32 ${item.type === 'stickers' ? 'object-contain' : 'object-cover'}`} loading="lazy" onError={(e) => {
                ...
                {(type === 'gif' || item.type === 'gifs' || item.type === 'stickers') && !imageUrl && (<div className="w-full h-32 flex items-center justify-center bg-muted text-muted-foreground text-xs">
                        No preview available
                    </div>)}
                {type === 'svg' && (<div className="svg-container w-full h-32 p-4">
                        <div dangerouslySetInnerHTML={{ __html: item.svg }}/>
                    </div>)}
                {type === 'emoji' && (<div className="flex items-center justify-center w-full h-32 text-4xl">
                        {getEmojiChar(item)}
                    </div>)}
                {type === 'background' && (<img src={item.path || item.url} alt={item.id || 'Background'} className="w-full h-32 object-cover" loading="lazy"/>)}
            </div>);
    }), []);
    return (<div className="space-y-2 max-w-4xl mx-auto p-0">
            ...
            {renderCollapsibleCard('giphy', 'GIF Search', '', (<div className={`panel-refresh ${gifLoading ? 'service-switching' : ''}`}>
                    {/* API Key Information for Imgur */}
                    {gifApi === 'imgur' && (<div className="mb-2 p-2 bg-blue-500/10 border border-blue-500/30 rounded text-xs">
                    ...
                    <form onSubmit={handleGifSearch} className="space-y-1">
                        ...
                        <div className="flex items-center gap-1">
                            <input type="text" value={gifQuery} onChange={(e) => setGifQuery(e.target.value)} placeholder="Search for GIFs..." className="flex-grow rounded-md border border-border bg-background px-1 py-1.5 text-xs focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-colors placeholder:text-muted-foreground" onFocus={() => setShowSuggestions(true)} onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}/>
                            <div className="relative">
                                <FaviconDropdown options={GIF_APIS} value={gifApi} onChange={handleServiceSwitch} className={`min-w-[100px] ${gifLoading ? 'service-dropdown-loading' : ''}`} accentColor={accentColor}/>
                                {/* Service switching indicator */}
                                {gifLoading && (<div className="absolute inset-0 bg-background/80 rounded flex items-center justify-center">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-xs">
                            ...
                            <div className="flex items-center gap-1">
                                <label className="flex items-center space-x-1 text-xs text-muted-foreground cursor-pointer group">
                                    <input type="radio" checked={searchFilters.contentType === 'gifs'} onChange={() => handleFilterChange('contentType', 'gifs')} className="appearance-none h-3 w-3 border-2 border-border rounded-sm bg-background
                                ...
                                <label className="flex items-center space-x-1 text-xs text-muted-foreground cursor-pointer group">
                                    <input type="radio" checked={searchFilters.contentType === 'stickers'} onChange={() => handleFilterChange('contentType', 'stickers')} className="appearance-none h-3 w-3 border-2 border-border rounded-sm bg-background
                                                   checked:bg-primary checked:border-transparent focus:outline-none 
                                                   focus:ring-2 focus:ring-offset-0 focus:ring-ring focus:ring-opacity-50
                                                   transition duration-150 group-hover:border-border"/>
                                    <span className="group-hover:text-foreground transition-colors duration-200 text-xs">
                            ...
                            {searchFilters.contentType === 'gifs' && (<div className="flex items-center gap-1">
                ...
                }} className={`text-xs bg-background border rounded px-1 py-0.5 min-w-[120px] pr-6 ${selectedCategory ? 'border-primary' : 'border-border'}`}>
                                            <option value="">All Categories</option>
                                            {gifCategories.length > 0 ? (gifCategories.map(cat => (<option key={cat.name} value={cat.name}>
                                                        {cat.name}
                                                    </option>))) : (<option value="" disabled>Loading categories...</option>)}
                                        </select>
                                        {selectedCategory && (<button type="button" onClick={() => {
                        setSelectedCategory('');
                        if (gifQuery.trim()) {
                    ...
                    }} className="absolute right-1 text-xs text-muted-foreground hover:text-destructive transition-colors" title="Clear category filter">
                                                ×
                                            </button>)}
                                    </div>
                                </div>)}
                            ...
                            <div className="relative">
                                <Button size="sm" variant="secondary" onClick={() => setShowBrowseOptions(!showBrowseOptions)} className="text-xs px-2 py-0.5 h-6">
                                    {showBrowseOptions ? 'Hide' : 'Browse'}
                                </Button>
                                
                                {/* Browse Options Dropdown */}
                                {showBrowseOptions && (<div className="absolute top-full left-0 mt-1 bg-card border border-border rounded shadow-lg z-10 min-w-[120px]">
                    ...
                    setShowBrowseOptions(false);
                            ...
                            <button type="button" onClick={() => setShowFilters(!showFilters)} className="text-primary hover:text-primary/80 transition-colors text-xs">
                                {showFilters ? 'Hide' : 'Show'} Advanced Filters
                            </button>

                            {/* Grid Icons Toggle */}
                            <button type="button" onClick={() => setShowGridIcons(!showGridIcons)} className="text-primary hover:text-primary/80 transition-colors text-xs">
                                {showGridIcons ? 'Hide' : 'Show'} Grid Icons
                            </button>
                        </div>

                        {/* Advanced Filters (Hidden by default) */}
                        {showFilters && (<div className="p-2 bg-card border border-border rounded text-xs space-y-2">
                                <div className="grid grid-cols-2 gap-2">
                                    {/* Content Rating */}
                                    <div>
                                        <label className="text-xs text-muted-foreground">Content Rating</label>
                                        <select value={searchFilters.rating} onChange={(e) => handleFilterChange('rating', e.target.value)} className="w-full text-xs bg-background border border-border rounded px-1 py-0.5">
                                            {CONTENT_RATINGS.map(rating => (<option key={rating.value} value={rating.value}>
                                                    {rating.label}
                                                </option>))}
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <select value={sortOrder} onChange={(e) => handleSortOrderChange(e.target.value)} className="w-full text-xs bg-background border border-border rounded px-1 py-0.5">
                                    ...
                                    {gifApi === 'tenor' && (<div>
                                            ...
                                            <select value={searchFilters.contentFilter} onChange={(e) => handleFilterChange('contentFilter', e.target.value)} className="w-full text-xs bg-background border border-border rounded px-1 py-0.5">
                                    ...
                                    {gifApi === 'tenor' && (<div>
                                            ...
                                            <select value={searchFilters.mediaFilter} onChange={(e) => handleFilterChange('mediaFilter', e.target.value)} className="w-full text-xs bg-background border border-border rounded px-1 py-0.5">
                                    ...
                                    {gifApi === 'tenor' && (<div>
                                            ...
                                            <select value={searchFilters.arRange} onChange={(e) => handleFilterChange('arRange', e.target.value)} className="w-full text-xs bg-background border border-border rounded px-1 py-0.5">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center">
                                            <input type="checkbox" checked={searchFilters.random} onChange={(e) => handleFilterChange('random', e.target.checked)} className="h-3 w-3 border border-border rounded bg-background checked:bg-primary"/>
                                    ...
                                    <div>
                                        ...
                                        <select value={searchFilters.limit} onChange={(e) => handleFilterChange('limit', parseInt(e.target.value))} className="w-full text-xs bg-background border border-border rounded px-1 py-0.5">
                        ...
                        {/* Search Suggestions */}
                        {showSuggestions && (<div className="p-1 bg-card border border-border rounded">
                                <div className="text-xs text-muted-foreground mb-1">Suggestions:</div>
                                <div className="flex flex-wrap gap-1">
                                    {getSearchSuggestions().slice(0, 8).map((suggestion, index) => (<button key={index} type="button" onClick={() => handleSuggestionClick(suggestion)} className="text-xs bg-muted hover:bg-muted/80 px-1 py-0.5 rounded transition-colors">
                                            {suggestion}
                                        </button>))}
                                </div>
                            </div>)}
                        ...
                        {/* Error Display */}
                        {searchError && (<div className="p-1 bg-destructive/10 border border-destructive/30 rounded">
                                <p className="text-destructive text-xs">{searchError}</p>
                            </div>)}

                        {/* Browse Options Panel - Show categories when browsing */}
                        {showBrowseOptions && showCategories && (<div className="p-2 bg-card border border-border rounded text-xs space-y-2">
                                <div className="flex flex-wrap gap-1">
                                    {gifCategories.map(cat => (<Button key={cat.name} size="sm" variant="secondary" onClick={() => handleCategoryClick(cat)}>
                    ...
                    {gifSearched && (<div className="mt-1">
                            {/* Service Switching Indicator */}
                            {gifLoading && gifResults.length === 0 && (<div className="mb-2 p-2 bg-accent/10 border border-accent/30 rounded text-xs">
                            ...
                            {selectedCategory && (<div className="mb-2 p-1 bg-primary/10 border border-primary/30 rounded text-xs">
                    ...
                    }} className="text-primary hover:text-destructive transition-colors" title="Clear category filter">
                                            Clear filter
                                        </button>
                                    </div>
                                </div>)}
                            
                            {gifLoading && gifResults.length === 0 ? (<div className="flex justify-center items-center py-4">
                                    ...
                                    {totalResults > searchFilters.limit && (<div className="flex justify-center items-center space-x-1 mb-2">
                                            <Button variant="secondary" size="sm" onClick={() => handleGifPageChange(gifPage - 1)} disabled={gifPage === 0 || gifLoading}>
                                                Previous
                                            </Button>
                                            <span className="text-xs text-muted-foreground">
                                                Page {gifPage + 1} of {Math.ceil(totalResults / searchFilters.limit)}
                                            </span>
                                            <Button variant="secondary" size="sm" onClick={() => handleGifPageChange(gifPage + 1)} disabled={gifPage >= Math.ceil(totalResults / searchFilters.limit) - 1 || gifLoading}>
                                                Next
                                            </Button>
                                        </div>)}
                                    ...
                                    <div className="grid grid-cols-4 gap-1">
                                        {getFilteredAndSortedResults().map((gif) => (<GridItem key={gif.id} item={gif} type="gif" onClick={() => {
                ...
                </div>), GIF_APIS.find(a => a.value === gifApi)?.domain)}

            {renderCollapsibleCard('svg', 'SVG Icons', '', (<div>
                    <form onSubmit={handleSvgSearch} className="flex items-center gap-1 mb-0.5">
                        <input type="text" value={svgQuery} onChange={(e) => setSvgQuery(e.target.value)} placeholder="Search for SVG icons..." className="flex-grow rounded-md border border-border bg-background px-1 py-1.5 text-xs focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-colors placeholder:text-muted-foreground"/>
                        <FaviconDropdown options={SVG_APIS} value={svgApi} onChange={setSvgApi} className="min-w-[100px]" accentColor={accentColor}/>
                    ...
                    <div className="grid grid-cols-4 gap-1">
                        {getPaginatedItems(svgResults, svgPage).map((result) => (<div key={result.name} className="relative group cursor-pointer p-1 bg-slate-800 rounded-md flex items-center justify-center aspect-square" onClick={() => setModalContent({ type: 'svg', data: result })}>
                                <div className="w-full h-full svg-container" dangerouslySetInnerHTML={{ __html: result.svg }}/>
                            </div>))}
                    </div>
                    {getTotalPages(svgResults) > 1 && (<div className="flex justify-center items-center space-x-1 mt-0.5">
            ...
            {renderCollapsibleCard('emoji', 'Emojis', randomHeaderEmoji, (<div>
                    ...
                    <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-1">
                        {getPaginatedItems(emojiResults, emojiPage).map((emoji, index) => (<div key={`${emoji.slug}-${index}`} className="relative group cursor-pointer p-1 bg-slate-800 rounded-md flex items-center justify-center aspect-square text-2xl sm:text-3xl md:text-4xl" onClick={() => setModalContent({ type: 'emoji', data: emoji })}>
                                {getEmojiChar(emoji)}
                            </div>))}
                    </div>
                    {getTotalPages(emojiResults) > 1 && (<div className="flex justify-center items-center space-x-1 mt-0.5">
            ...
            {renderCollapsibleCard('backgrounds', 'Backgrounds', '🖼️', (<div>
                    <form onSubmit={handleBackgroundSearch} className="flex items-center gap-1 mb-0.5">
                        <input type="text" value={backgroundQuery} onChange={(e) => setBackgroundQuery(e.target.value)} placeholder="Search for backgrounds..." className="flex-grow rounded-md border border-border bg-background px-1 py-1.5 text-xs focus:ring-2 focus:ring-accent focus:border-accent outline-none transition-colors placeholder:text-muted-foreground"/>
                        <FaviconDropdown options={BACKGROUND_APIS} value={backgroundApi} onChange={setBackgroundApi} className="min-w-[100px]" accentColor={accentColor}/>
                        <Button type="submit" disabled={backgroundLoading || !backgroundQuery.trim()} size="sm">{backgroundLoading ? 'Searching...' : 'Search'}</Button>
                    </form>
                    
                    {/* API Key Information */}
                    {['unsplash', 'pexels', 'pixabay', 'deviantart'].includes(backgroundApi) && (<div className="mb-2 p-2 bg-blue-500/10 border border-blue-500/30 rounded text-xs">
                            <div className="flex items-start gap-2">
                                <span className="text-blue-500">🔑</span>
                                <div>
                                    <p className="text-blue-500 font-medium mb-1">
                    ...
                    {/* Free Services Info */}
                    {['wallhaven', 'artstation'].includes(backgroundApi) && (<div className="mb-2 p-2 bg-green-500/10 border border-green-500/30 rounded text-xs">
                            <div className="flex items-start gap-2">
                ...
                return (<div key={bg.id} className="relative group cursor-pointer bg-slate-800 rounded-md overflow-hidden" onClick={() => setModalContent({ type: 'background', data: { ...bg, preview: bg.thumbs?.large || bg.thumbs?.original, full: bg.path } })}>
                                    {imageUrl && (<img src={imageUrl} alt={bg.id} className="w-full h-16 object-cover"/>)}
                    ...
                    {getTotalPages(backgroundResults) > 1 && (<div className="flex justify-center items-center space-x-1 mt-0.5">
            ...
            {modalContent && (<Modal isOpen={!!modalContent} onClose={() => setModalContent(null)} title={modalContent.type === 'gif' ? modalContent.data.title :
                modalContent.type === 'svg' ? modalContent.data.name :
                    modalContent.type === 'background' ? modalContent.data.id :
                        'Emoji Preview'} actions={getModalActions(modalContent.type, modalContent.data)}>
                    ...
                    {modalContent.type === 'background' && (renderBackgroundModal(modalContent.data))}
                    {modalContent.type === 'sticker' && (<div className="p-4 bg-transparent rounded-md flex justify-center items-center aspect-square max-w-xs mx-auto modal-sticker-preview">
                            <img src={modalContent.data.images?.original?.url || modalContent.data.png_url} alt={modalContent.data.title || modalContent.data.name} className="max-w-full max-h-full object-contain" onError={e => {
