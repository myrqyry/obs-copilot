...
export const useObsActions = ({ obsService, obsData, onRefreshData, onAddMessage, setErrorMessage }) => {
    const handleObsAction = useCallback(async (action) => {
        ...
        try {
            switch (action.type) {
                case 'createInput':
                    ...
                    await obsService.createInput(createAction.inputName, createAction.inputKind, createAction.inputSettings, sceneToAddTo, createAction.sceneItemEnabled);
                    actionFeedback += `\n✅ Successfully created input "${createAction.inputName}" of kind "${createAction.inputKind}".`;
                    return { success: true, message: `Successfully created input "${createAction.inputName}" of kind "${createAction.inputKind}".` };
                case 'setInputSettings':
                    const setSettingsAction = action;
                    await obsService.setInputSettings(setSettingsAction.inputName, setSettingsAction.inputSettings, setSettingsAction.overlay);
                    actionFeedback = `\n✅ Successfully updated settings for input "${setSettingsAction.inputName}".`;
                    return { success: true, message: `Successfully updated settings for input "${setSettingsAction.inputName}".` };
                case 'setSceneItemEnabled':
                    ...
                    return { success: true, message: `Successfully ${enabledValue ? 'enabled' : 'disabled'} "${targetAction.sourceName}" in scene "${targetAction.sceneName}".` };
                case 'getInputSettings':
                    ...
                    return { success: true, message: `Fetched settings for input "${getSettingsAction.inputName}".` };
                case 'getSceneItemList':
                    ...
                    return { success: true, message: `Fetched items for scene "${getListAction.sceneName}".` };
                case 'setCurrentProgramScene':
                    ...
                    return { success: true, message: `Successfully switched to scene "${setSceneAction.sceneName}".` };
                ...
                case 'createScene':
                    ...
                    return { success: true, message: `Successfully created scene "${createSceneAction.sceneName}".` };
                case 'removeInput':
                    ...
                    return { success: true, message: `Successfully removed input "${removeInputAction.inputName}".` };
                case 'setSceneItemTransform':
                    const transformAction = action;
                    const sceneItemIdTransform = await obsService.getSceneItemId(transformAction.sceneName, transformAction.sourceName);
                    ...
                    await obsService.setSceneItemTransform(transformAction.sceneName, sceneItemIdTransform, transformAction.transform);
                    actionFeedback = `\n✅ Successfully updated transform for "${transformAction.sourceName}" in scene "${transformAction.sceneName}".`;
                    return { success: true, message: `Successfully updated transform for "${transformAction.sourceName}" in scene "${transformAction.sceneName}".` };
                case 'createSourceFilter':
                    const filterAction = action;
                    await obsService.createSourceFilter(filterAction.sourceName, filterAction.filterName, filterAction.filterKind, filterAction.filterSettings);
                    actionFeedback = `\n✅ Successfully created filter "${filterAction.filterName}" on source "${filterAction.sourceName}".`;
                    return { success: true, message: `Successfully created filter "${filterAction.filterName}" on source "${filterAction.sourceName}".` };
                case 'setInputVolume':
                    ...
                    return { success: true, message: `Successfully set volume for input "${volumeAction.inputName}".` };
                case 'setInputMute':
                    ...
                    return { success: true, message: `Successfully ${muteAction.inputMuted ? 'muted' : 'unmuted'} input "${muteAction.inputName}".` };
                ...
                case 'triggerStudioModeTransition':
                    ...
                    return { success: true, message: `Audio monitoring for "${monitorAction.inputName}" set to "${monitorAction.monitorType}".` };
                case 'setInputAudioMonitorType':
                    ...
                    return { success: true, message: `Blend mode for "${blendAction.sourceName}" set to "${blendAction.blendMode}".` };
                ...
                case "toggleStudioMode":
                    ...
                    actionFeedback = "\n✅ Studio mode toggled!";
                    return { success: true, message: `Studio mode ${action.enabled ? "enabled" : "disabled"}!` };
                ...
                case "triggerHotkeyByKeySequence":
                    ...
                    return { success: true, message: `Output status for "${getOutputStatusAction.outputName}" fetched.` };
                ...
                case 'getSourceFilterList': {
                    ...
                    return { success: true, message: `Fetched filter settings for "${filterName}" on source "${sourceName}".` };
                }
                case 'getSourceFilterSettings': {
                    ...
                    return { success: true, message: `Fetched default settings for filter kind "${filterKind}".` };
                }
                case 'getSourceFilterDefaultSettings': {
                    ...
                    return { success: true, message: `Set filter index for "${filterName}" on source "${sourceName}" to ${filterIndex}.` };
                }
                case 'setSourceFilterIndex': {
                    ...
                    return { success: true, message: `Renamed filter "${filterName}" to "${newFilterName}" on source "${sourceName}".` };
                }
                case 'setSourceFilterName': {
                    ...
                    return { success: true, message: `Duplicated filter "${filterName}" as "${newFilterName}" on source "${sourceName}".` };
                }
                case 'duplicateSourceFilter': {
                    ...
                    return { success: true, message: `Removed filter "${filterName}" from source "${sourceName}".` };
                }
                ...
                case 'getVideoSettings': {
                    ...
                    return { success: true, message: `Fetched transform for "${sourceName}" in scene "${sceneName}".` };
                }
                case 'getSceneItemTransform': {
                    ...
                    return { success: true, message: `Fetched filter "${filterName}" on source "${sourceName}".` };
                }
                case 'getSourceFilter': {
                    ...
                    return { success: true, message: `${filterEnabled ? 'Enabled' : 'Disabled'} filter "${filterName}" on source "${sourceName}".` };
                }
                case 'setSourceFilterEnabled': {
                    ...
                    return { success: true, message: `Updated settings for filter "${filterName}" on source "${sourceName}".` };
                }
                ...
                case 'getReplayBufferStatus': {
                    ...
                    return { success: true, message: `Duplicated "${sourceName}" from scene "${sceneName}" to scene "${targetScene}".` };
                }
                ...
                case 'getCurrentSceneCollection': {
                    ...
                    return { success: true, message: `Switched to scene collection "${sceneCollectionName}".` };
                }
                ...
                default:
                    const unknownActionType = action.type;
            ...
            const failureFeedback = `\n❗ Failed to execute OBS action "${action.type}": ${err.message || 'Unknown error'}`;
            actionAttemptMessage += `${failureFeedback}`;
            onAddMessage({ role: 'system', text: actionAttemptMessage });
            setErrorMessage(`OBS Action "${action.type}" failed: ${err.message}`);
        }
