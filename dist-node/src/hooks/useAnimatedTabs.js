...
export const useAnimatedTabs = (activeTab) => {
    const tabBarRef = useRef(null);
    const tabRefs = useRef(new Map());
    // Register a tab element
    const registerTab = useCallback((tabKey, element) => {
        if (element) {
    ...
    const handleTabClick = useCallback((tabKey, onClick) => {
        return (event) => {
            ...
            // Animate the click
            gsap.fromTo(tabElement, { scale: 1 }, {
                scale: 0.95,
                duration: 0.1,
                ease: "power2.out",
                yoyo: true,
            ...
            gsap.fromTo(tabElement, { boxShadow: "0 0 0 rgba(203, 166, 247, 0)" }, {
    ...
    const handleTabHover = useCallback((isEntering) => {
        return (event) => {
            const tabElement = event.currentTarget;
