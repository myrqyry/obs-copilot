...
export async function generateChuteImage({ prompt, guidance_scale = 7.5, width = 1024, height = 1024, num_inference_steps = 10, seed = null, apiKey, }) {
    const apiToken = (apiKey && apiKey.trim().length > 0)
        ? apiKey.trim()
        : (import.meta.env.VITE_CHUTES_API_TOKEN || window?.VITE_CHUTES_API_TOKEN);
    if (!apiToken) {
